...
1. Strato unico totalmente connesso (_FC: fully connected_)
2. Due strati _FC_
...
Le reti con maggiore successo (reti vincolate con pesi condivisi) avevano la maggiore capacità di generalizazione, e formarono la base per le CNN moderne. Il modello con uno strato unico _FC_, invece, tende a sovraggiustarsi (_overfitting_).
...
+ 12 (5$\times $5) kernel con _stride_ 2 (avanzamento di 2 pixel): il prossimo strato avrà una risoluzione inferiore
...
<center>
    ...
    <b>Fig. 1</b> Architettura rete convoluzionale
</center>

Il seguente è un esempio di una rete con un solo strato convoluzionale:
1. Si consideri un input con dimensione *32*$\times$*32*
2. Lo strato convoluzionale passa un kernel di dimensione *5*$\times$*5* con _stride_ 1 sull'immagine, creando una mappa delle caratteristiche *28*$\times$*28*
3. Si passa la mappa delle caratteristiche ad una funzione non lineare: dimensione *28*$\times$*28*
4. Si passa ad uno strato di _pooling_, che calcola la media su una finestra di dimensione *2*$\times$*2* con _stride_ 2: dimensione *14*$\times$*14*
5. Si ripetono 1-4 per ciascuno dei 4 kernel
...
Generalmente il primo strato, fatto di semplici combinazioni di convoluzioni/_pooling_, rileva caratteristiche semplici, come bordi orientati. Dopo il primo strato di convoluzioni/_pooling_, l'obiettivo è rilevare combinazioni di caratteristiche dagli strati precedenti. Per ottenere questo si ripetono i passi 2-4, passando diversi kernel sulle mappe delle caratteristiche degli strati precedenti, che vengono poi sommati per formare una nuova mappa delle caratteristiche:
...
1. Si passa un nuovo kernel *5*$\times$*5* su ciascuna mappa delle caratteristiche dagli strati precedenti e si sommano i risultati (Nota: nell'esperimento del 1989 del Prof. LeCun, si omettono alcuni strati per motivi computazionali. Le configurazioni moderne generalmente impongono che la rete sia completamente connessa): dimensione *10*$\times$*10*
2. Si passa il risultato dello strato convoluzionale ad una funzione non lineare: dimensione *10*$\times$*10*.
3. Si ripetono 1-2 per ciascuno dei 16 kernel.
4. Si passa il risultato ad uno strato di _pooling_, che calcola la media su una finestra di dimensione *2*$\times$*2* con _stride_ 2: dimensione *5*$\times$*5* per ogni mappa delle caratteristiche.
...
Per generare un _output_, l'ultimo strato convoluzionale somiglia ad una connessione completa (_FC_):
...
1. L'ultimo strato convoluzionale passa un _kernel_ *5*$\times$*5* sopra l'intera mappa delle caratteristiche, le quali vengono sommate, risultando in una matrice di dimensione *1*$\times$*1*
2. Si passa il risultato ad una funzione non lineare: dimensione *1*$\times$*1*.
3. Si genera un singolo _output_ per la categoria in questione.
...
### Equivarianza rispetto ai cambi di posizione
<!-- ### Shift equivariance-->

<center>
    <img src="{{site.baseurl}}/images/week03/03-2/shift_invariance.gif" width="600px" /><br>
    <b>Fig. 2</b> Equivarianza rispetto ai cambi di posizione
</center>

Come dimostrato dall'animazione sugli slide (questo è un'altro esempio), la translazione dell'_input_ risulta nella stessa _translazione_ della mappa delle caratteristiche. Tuttavia, il cambiamento nella mappa delle caratteristiche vengono dilatate o contratte rispetto alle operazioni di convoluzione/_pooling_. Per esempio, se si ha uno strato di _pooling_ 2$\times$2 e stride 2, una traslazione di 1 pixel dell'_input_ risulta in una traslazione di 0.5 pixel delle mappe delle caratteristiche successive. La risoluzione viene quindi "scambiata" per avere un maggior numero di tipi di caratteristiche, ovvero per rendere la rappresentazione più astratta e meno sensibile a traslazioni e distorsioni.
...
* **Normalizzazione**
  * Aggiustamento _whitening_ (facoltativo)
  * Metodi sottrattivi, ad esempio sottrazione della media, filtro passa-alto
  * Divisive: _local contrast normalisation_, normalizzazione della varianza
...
## [LeNet5 e la rilevazione delle cifre](https://www.youtube.com/watch?v=FW5gFiJb-ig&t=3830s)
...
1. _Log-softmax_
2. Strato _FC_ di dimensione 500$\times$10
3. _ReLU_
4. Strato _FC_ di dimensione 4$\times$4$\times$50)$\times$500
5. _Max Pooling_ di dimensione 2$\times$2, stride di 2.
6. _ReLu_
7. Convoluzione con 20 canali _output_, _kernel_ 5$\times$5, e stride di 1.
8. _Max Pooling_ di dimensione 2$\times$2, stride di 2.
9. _ReLu_
10. Convoluzione con 20 canali _output_, _kernel_ 5$\times$5, stride di 1.
...
Anche se `fc1` e `fc2` sono strati completamente connessi (_FC_), si possono consdierare come strati convoluzionali con _kernel_ che ricoprono l'intero _input_. Gli strati _FC_ vengono utilzizati per motivi di efficenza.

Lo stesso codice si può scrivere utilizzando `nn.Sequential`, ma questo metodo è stato dismesso.
...
In una rete completamente convoluzionale, non vi è bisogno di specificare le dimensioni dell'_input_. Tuttavia, cambiare la dimensione dell'_input_ cambia la dimensione dell'_output_.

Si consideri un sistema di rilevazione di testo scritto a mano in cosrivo. Non è necessario segmentare l'immagine _input_. Basta utilizzare una CNN su l'immagine intera: i _kernel_ copriranno ogni posizione sul'immagine intera e segneranno lo stesso _output_ indipendentemente dalla posizione in cui è stata rilevato lo schema (_pattern_). Applicare la CNN ad un'immagine intera è molto meno costoso che applicarla su diverse posizioni separatamente. Non serve segmentazione iniziale, il che è un sollievo perché la segmentazione di un'immagine è simile al riconosimento di un immagine.
...
LeNet5 è stato addestrato su immagini 32$\times$32 di MNIST per classificare cifre individuali al centro di un'immagine. Si utilizzò una technica di aumento dei dati, spostando le cifre, cambiando la dimensione delle cifre, ed inserendo altre cifre a fianco. Fu anche addestrato utilizzando un'undicesima categoria, per rappresentare cifre che non corrispondevano alle 10 cifre precedenti. Le immagini etichettate (_labeled_) da questa categorie erano generate producendo immagini vuote, oppure collocando le cifre ai lati invece che al centro dell'immagine.
...
<center>
 ...
 L'immagine qui sopra dimostra che una rete LeNet5 addestrata su immagini 32$\times$32 può essere utilizzata su un'immagine input 32$\times$64 per rilevare cifre in diverse posizioni.
...
## [Problema dell'unione delle caratteristiche](https://www.youtube.com/watch?v=FW5gFiJb-ig&t=4827s)

### Cos'è il problema dell'unione delle caratteristiche?
<!--### What is the feature binding problem?-->

I neuroscienziati visivi e gli esperti di visione artificiale affrontano il problema di definire un oggetto come tale. Un oggetto è una collezione di caratteristiche, ma come si possono unire tutte le caratteristiche per costruire l'oggetto in questione?
...
Possiamo risolvere il problema dell'unione delle caratteristiche utilizzando una semplice CNN: bastano solo due strati convoluzionali con _pooling_ e altri due strati _FC_ senza meccanismi specifici, ponendo che si abbiamo abbastanza funzioni non lineari e dati per addestrare la rete.
...
L'animazione qui sopra mostra la capacità delle CNN nel rilevare le cifre tramite lo spostamento di un unico tratto, dimostrando come risolvono il problema dell'unione delle caratteristiche, rilevando le caratteristiche in maniera gerarchica e compositiva.
...
### Esempio: _input_ di lunghezza dinamica
<!-- ### Example: dynamic input length-->

Possiamo costruire una CNN con due strati convoluzionali con stride 1 e due strati _pooling_ con stride 2 in modo che lo stride totale sia 4. Cosí, per ottenere un nuovo _output_ bisogna traslare la finestra di _input_ di 4. Più esplicitamente, si può vedere dalla figura seguente (unità verdi). Iniziamo con un _input_ di dimensione 10, si realizza una convoluzione di dimensione 3 per ottenere 8 unità. Dopodiché, si realizza il _pooling_ di dimensione 2 per ottenere 4 unità. Come prima, si ripetono le operazioni di convoluzione e  _pooling_ fino ad ottenere 1 unità di output.
...
Poniamo di aggiungere 4 unità allo strato _input_ (unità rosa nella figura qui sopra) in modo da ottenere 4 unità in più dopo il primo strato convoluzionale, 2 unità in più dopo la prima operazione di _pooling_, 2 unità in più dopo il secondo strato convoluzionale, e 1 unità in più nell'_output_. Di consequenza, la dimensione della finestra  per generare un nuovo output dovrà essere 4 (2 stride $\times$2). Inoltre, questo indica che aumentando la dimensione dell'_input_ si aumenta la dimensione di ciascuno strato, dimostrando che le CNN riescono a gestire _input_ con lunghezza dinamica.
...
2. **Stazionarità**: La seconda proprietà è che le caratteristiche sono essenziali e possono apparire ovunque sull'immagine, giustificando l'utilizzo dei pesi condivisi e del _pooling_. Inoltre, i segnali statistici seguono una distribuzione uniforme, quindi bisogna ripetere il rilevamento delle caratteristiche per ogni posizione sull'immagine input.
...
3. **Composizionalità**: La terza proprietà è che le immagini naturali sono compositive, quindi le caratteristiche formano un'immagine in maniera gerarchica. Questo giustifica l'utilizzo di più strati di neuroni, che oltretutto coincide con la ricerca di  Hubel e Weisel sulle cellule semplici e complesse.
