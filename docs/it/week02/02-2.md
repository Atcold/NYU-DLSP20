...
## [Esempio concreto di retropropagazione e introduzione ai moduli basilari di reti neurali](https://www.youtube.com/watch?v=d9vdh3b787Y&t=3022s)


### Esempio

Successivamente considereremo un esempio concreto di retropropagazione accompagnato da un grafico.
...
Nota che delle complicazioni potrebbero sorgere quando l'architettura del grafo non è fissa, ma dipende dai dati. Per esempio, potremmo fissare un modulo di una rete neurola che dipende dalla lunghezza del vettore in input. Anche se questo è possibile, diventa di difficoltà sempre superiore gestire questa variazione quando il numero dei cicli eccede una quantità ragionevole


### Moduli base di reti neurali

Esistono diversi dipi di moduli pre-costruiti oltre ai familiari moduli Linear e ReLU. Questi sono utili perché sono ottimizzati individualmente per eseguire le loro rispettive funzione (invece di essere costruiti da una combinazione di altri moduli elementari)
...
- Duplicate: $Y_1=X$, $Y_2=X$
  ...
  - Simile a "Y - splitter" dove entrambi gli outpu sono uguali all'input.
...
Tuttavia, l'uso di softmax lascia la rete suscettibile ai gradienti evanescenti. I gradienti evanescenti sono un problema, impedisce ai pesi a valle di essere modificati dalla rete neurale, il che potrebbe fermare completamente la rete da ulteriori allenamenti.
...
## [Trucchi pratici per la retropropagazione](https://www.youtube.com/watch?v=d9vdh3b787Y&t=4924s)


### Usare ReLU come funzione d'attivazione non lineare

ReLu funziona al meglio nelle reti con molti strati, il che ha causato alle alternative come la funzione sigma e alla tangente iperbolica $\tanh(\cdot)$ di cadere a suo favore. La ragione per cui la ReLu funziona meglio è dovuta alla singola pecularità che fa si che scali in modo equivariante.


### Uso della perdità di entropia incrociata come funzione obiettivo per problemi di classificazione

Log softmax, come discusso nella lezione precedentemente, è un caso speciale di perdità di entropia-incrociata. In PyTorch, bisogna assicursarsi di fornire una funzione di perdità di entropia-incrociata con *log* softmax come input (al contrario della normale softmax).


### Uso della discesa stocastica del gradiente come minibatche durante l'allenamento
...
Ci sono vari trucchi per inizializzare i pesi in PyTorch. Uno di questi trucchi che funziona bene per i modelli profondi è l'inizializzazione Kaiming, dove la varianza dei pesi è inversamente proporzionale alla radice quadrata del numero di input.
...
Infine, nota che la retropropagazione non funziona solo per i modelli impilati; può funzionareper ogni grafo diretto aciclico (DAG) finché vi è un ordine parziale dei moduli.
