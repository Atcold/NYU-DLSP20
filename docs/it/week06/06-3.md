...
-->
La figura sopra è una tipica architettura RNN. Per eseguire la rotazione sui passaggi precedenti in RNN, utilizziamo le matrici, che possono essere considerate frecce orizzontali nel modello sopra. Poiché le matrici possono modificare le dimensioni degli output, se il determinante che selezioniamo è maggiore di 1, il gradiente si gonfia nel tempo e provoca un'esplosione del gradiente. Relativamente parlando, se l'autovalore che selezioniamo è piccolo su 0, il processo di propagazione ridurrà i gradienti e porterà alla scomparsa del gradiente.
...
-->
Nei tipici RNN, i gradienti saranno propagati attraverso tutte le possibili frecce, il che fornisce ai gradienti una grande possibilità di scomparire o esplodere. Ad esempio, il gradiente al tempo 1 è grande, indicato dal colore brillante. Quando attraversa una rotazione, il gradiente si restringe molto e al passo temporale 3 viene ucciso.
...
-->
Un ideale per evitare che le pendenze esplodano o svaniscano è quello di saltare le connessioni. A tale scopo, è possibile utilizzare moltiplicare le reti.
...
Nel caso sopra, abbiamo diviso la rete originale in 4 reti. Prendi la prima rete per esempio. Prende un valore dall'input al tempo 1 e invia l'output al primo stato intermedio nel layer nascosto. Lo stato ha altre 3 reti in cui $\circ$s consente il passaggio dei gradienti mentre $-$s blocca la propagazione. Tale tecnica si chiama gated recurrent network (rete ricorrente "con un cancello")

<!--
LSTM is one prevalent gated RNN and is introduced in detail in the following sections.
-->
LSTM è la principale RNN gated ed è introdotta in dettaglio nelle sezioni seguenti.
...
-->
Di seguito sono riportate le equazioni che descrivono una LSTM. La porta di input è evidenziata da caselle gialle, che saranno una trasformazione affine. Questa trasformazione di input moltiplicherà $c [t]$, che è il nostro gate candidato.
