...
* ** محلي ** العصبون في الشبكة العصبية يكون متصلا بالطبقات المجاورة فقط، ولكن ليس بجميع الطبقات في الشبكة. نسمي هذا افتراض مجال الاستقبال المحلي.
...
* ** هرمي ** سيتم دمج ميزات المستوى المنخفض لتشكيل ميزات متوسطة المستوى. بعد ذلك ، سيتم دمج ميزات المستوى المتوسط هذه بشكل تدريجي لتشكيل ميزات ذات مستوى أعلى وأعلى. على سبيل المثال ، تمثيل مرئي.
...
* جمل ، كلمات ، كلام الكامن في المجال الإقليدي 1D. على سبيل المثال يمكن تمثيل كل حرف بعدد صحيح.
...
لنأخذ على سبيل المثال شبكة اجتماعية. يتم التقاط الشبكة الاجتماعية بشكل أفضل من خلال تمثيل المخططات لأن الاتصال الثنائي بين مستخدمين لا يشكل شبكة. تمثل عُقد مخططات المستخدمين ، بينما تمثل الحواف بين عقدتين الوصلة بين عقدتين (مستخدمين). لكل مستخدم مصفوفة ميزات ثلاثية الأبعاد تحتوي على رسائل، صور، ومقاطع فيديو.
...
المصفوفة $Phi\$ كما هي معرفة أدناه ، تحتوي على متجهات أعمدة، أو متجهات ذاتية من$phi‪_‬1\$ إلى $phi_n\$ ، كل منها بحجم $ n \times 1 $ ، وتسمى هذه أيضًا دوال فورييه. وتشكل دوال فورييه أساسًا متعامدًا.
...
الصورة أدناه تبين المتجهات الذاتية لـ لابلاسيان إقليدي 1D.
...
بالنسبة لمجال المخطط ، من اليسار إلى اليمين ، هو الأول،الثاني، الثالث،‪...‬ وظائف فورييه للمخطط. على سبيل المثال ، يحتوي $phi_1\$ على تذبذبات ذات قيم موجبة (حمراء) وسالبة (زرقاء) ، نفس القيم $phi‪_‬2$ ، $ \phi‪_‬3 $. تعتمد هذه التذبذبات على طوبولوجيا المخطط، والتي تتعلق بهندسة المخططات مثل المجتمعات،المحاور، وما إلى ذلك ، وهي مفيدة للتجميع في المخططات. انظر أدناه.
...
تحويلات فورييه هي عمليات خطية، محصلة جداء مصفوفة $Phi ^ {T}\ $ بمتجه $ h $.
