...
حاليا طريقة SGD مع الزخم هي أفضل طرق التحسين لكثير من مشاكل تعلم الآلة. لكن هنالك طرق أخرى - يطلق عليها عموماً الطرق التَكَيُّفية و تم تطويرها عبر السنين - تكون مفيدة في المشاكل الصعبة و التي يكون فيها استخدام SGD لا يؤدي إلى نتايجة جيدة.  
...
في طريقة SGD فإن كل وزن في الشبكة يتم تحديثه باستخدام معادلة بمعدل تعلم ثابت ($\gamma$). لكن في الطرق التَكَيُّفية فيتم *تكييف معدل تعلم لكل وزن*. لهذا السبب فأن المعلومات التي نحصل عليها من إنحدار كل وزن يتم استخدامها.  
...
عملياً فإن الشبكات في الأغلب تكون بنيتها مختلفة في مختلف أجزاءها. مثلاً الأجزاء الأولى من الشبكات العصبية الإلتفافية (CNN) قد تتكون من طبقات إلتفافية عدد قنواتها قليل و أبعاد القناة كبيرة و لكن في الأجزاء الأخيرة قد تكون الطبقات عدد قنواتها أكثر وأبعاد القناة صغيرة. فالعمليات تكون مختلقة في مختلف أجزاء الشبكة لذلك فإن معدل التعلم قد يكون مناسب للأجزاء الأولى و غير مناسب للأجزاء الأخيرة. و هذا يعني أن استخدام معدلات تعلم تَكَيُّفية قد يكون مفيد. 
...
الفكرة الأساسية ل*إنتشار جذر تربيع المتوسط* (RMSprop, Root Mean Square Propagation) هي أن الإنحدار يتم تسويته باستخدام جذر تربيع المتوسط. 
...
حيث $\gamma$ هي معدل التعلم العام و  $\epsilon$ هي قيمة تكون من مضاعفات $\epsilon$ $10^{-7}$ أو $10^{-8}$ -- و تستعمل من أجل تجنب القسمة على صفر و $v_{t+1}$ هي عبارة عن مقدر العزم الثاني. 
...
الطريقة الأصلية تبقي متوسط أسي متحرك لعزم ثاني غير مركزي لذلك لا نطرح المتوسط هنا. *العزم الثاني* يستخدم لتسوية الإنحدار و هذا يعني أن كل عنصر في الإنحدار يتم قسمته على الجذر التربيعي للعزم الثاني. إذا كانت القيمة المتوقعة للإنحدار صغيرة فأن هذه العملية مشابهة لقسمة الإنحدار على الانحراف المعياري. 

<!-- 
Using a small $\epsilon$ in the denominator doesn't diverge because when $v$ is very small, the momentum is also very small. 
-->

استخدام قيمة صغيرة ل$\epsilon$  في المقام لا يؤدي إلى تباعد لأن عندما تكون $v$ صغيرة جداً فأن الزخم يكون صغير جداً.
...
ADAM ضرورية في تدريب بعض الشبكات مثل التي تستخدم لتكوين نماذج اللغات. من أجل تحسين الشبكات العصبية SGD مع الزخم أو ADAM عموماً يفضل إستخدامها. لكن الجانب النظري لADAM غير مفهوم بصورة جيدة و لدى ADAM عدة مساوىء منها: 

* يمكن أن يثبت عن طريق إختبار بسيط أن الطريقة لا تتقارب لقيمة معينة 
* معروف أن الطريقة تعطي أخطاء تعميم. إذا تم تدريب الشبكة العصبية لتعطي خسارة صفرية في البيانات التي تم تدريبها عليها فأنها لن تعطي خسارة صفرية على البيانات التي لم تراها الشبكة. أيضاً شائع - تحديداً في مشاكل الصور - أن تعطي أخطاء تعميم أسوء من عندما تستخدم SGD. الأسباب قد تتضمن مثلاً أنها تجد أقرب نقطة صغرى محلية أو أن هنالك تشويش أقل في ADAM أو من بنيتها.  
* في ADAM نحتاج لإستخدام 3 مستودعات بينما في SGD نحتاج إلى إثنان فقط. هذا غير مؤثر إلا عندما ندرب نموذج حجمه عدة قيقابايت و في هذه الحالة قد تكون الذاكرة غير كافية. 
...
في الشبكات العصبية فأننا ننتقل بين عمليات خطية وعمليات غير خطية. العمليات الغير خطية تعرف أيضاً بدوال التنشيط مثلاً ReLU. يمكننا وضع طبقات التسوية قبل الطبقات الخطية أو بعد الطبقات الغير خطية. أكثر مكان شائع لوضعها بين الطبقات الخطية و دوال التنشيط كما موضح في الشكل أدناه.  
...
حيث $x$ هو متجه الإدخال و $y$ هو متجه الإخراج و $\mu$ هي المقدر لمتوسط $x$ و $\sigma$ هي المقدر للإنحراف المعياري ل$x$ و $a$ هي معامل تكبير قابل للتعلم و $b$ هي معامل إنحياز قابل للتعلم.    
...
- *تسوية حزمة* (Batch norm): التسوية يتم تطبيقها فقط على قناة واحدة من المدخل. هذه أول طريقة تم إقتراحها و أكثر الطرق شيوعاً. رجاءاً إقرأ هذا المقال [How to Train Your ResNet 7: Batch Norm](https://myrtle.ai/how-to-train-your-resnet-7-batch-norm/) لتحصل على معلومات أكثر. 
...
<center>
<img src="{{site.baseurl}}/images/week05/05-2/5_2_conv_xkcd.png" style="zoom:60%"><br>
<b>شكل 5:</b> أحياناً إنها فعلاً تعمل ! 
</center>
...
في مشكلة إعادة تكوين صور الMRI البيانات المجردة تؤخذ من جهاز الرنين المغنطيسي و يتم إعادة تكوين الصورة باستخدام خوارزمية. أجهزة الMRI تلتقط البيانات في مجال فورير ثنائي الأبعاد صف واحد أو عمود واحد في المرة الواحدة (كل عدد قليل من الملي ثواني). هذا المدخل يتكون من تردد و قناة طور و القيمة تمثل مقدار دالة جيبية لديها نفس التردد و الطور. ببساطة يمكن أن نعتبرها صورة قيمها أعداد مركبة و تحتوي على قناة حقيقية و قناة تخيلية. إذا طبقنا تحويل فورير العكسي على هذا المدخل - عن طريق جمع كل الموجات الجيبية موزونة بقيمها - فأننا سنحصل على الصورة الأصلية. 
...
يوجد تحويل خطي من مجال فورير إلى مجال الصورة و كفاءته جيدة و يأخذ ميلي ثواني بغض النظر عن كبر حجم الصورة. لكن السؤال هل يمكن أن نحصل علىي سرعة أكبر.  
...
المشكلة الجديدة التي تحتاج إلى أن تحل هي تسريع الMRI و نعني بالتسريع أن نجعل عملية إعادة تكوين الMRI أسرع. و نريد  أن نحافظ على جودة الصورة. واحدة من الطرق و من الطرق الأكثر نجاحاً هي أن لا نلتقط جميع الأعمدة من ماسح الMRI. يمكننا أن نتجاهل بعض الأعمدة عشوائياً لكن عملياً إلتقاط الأعمدة الموجودة في الوسط أكثر فائدة  ﻷنها تحتوي على معلومات كثيرة و يمكن أن نختار عشوائياً خارج المنطقة الوسطى. تكون المشكلة أنه لا يمكننا إستعمال التحويل الخطي لإعادة تكوين الصورة مرة أخرى. الصورة الموجودة على أقصى اليمين في شكل 7 توضح مخرج التحويل الخطي المطبق على فضاء فورير. واضح أن هذه الطريقة لا تعطينا مخرجات مفيدة جداً و هنالك مجال لتحسينها.  
...
###  Compressed sensing (الأستشعار المضغوط)

 واحدة من أكبر الأختراقات في مجال الرياضيات النظرية لوقت طويل هي الأستشعار المضغوط. ورقة علمية بواسطة  <a href="https://arxiv.org/pdf/math/0503066.pdf">Candes et al.</a> توضح نظرياً  أنه يمكن الحصول صورة مثالية معاد تكوينها من مجال فورير. بصورة أخرى فأن الموجة التي نحاول أن نعيد تكوينها تكون منتشرة أو بنيتها منتشرة لكن يمكننا أن نعيد تكوينها بصورة مثالية من عدة قياسات قليلة. لكن توجد متطلبات عملية لكي تعمل هذه 
الطريقة -- لا نحتاج لأخذ عينات عشوائياً لكن نحتاج لأخذ عينات بصورة غير منتظمة -- لكن عملياً الناس يأخذون عينات عشوائية. بالأضافة أخذ عينة من عمود كامل أو نص عمود يحتاج إلى زمن لذلك عملياً نأخذ عينات من أعمدة كاملة. 

و نحتاج إلى شرط أخر و هو أن يوجد إنتشار (sparsity) في الصورة و نعني بالإنتشار  أن  تكون في الصورة بيكسلات صفرية أو سوداء كثيرة. المدخل يمكن أن نمثله إنتشارياً عن طريق مفكوك الطول الموجي  لكن حتى هذا المفكوك يعطينا إنتشاراً مقرباً و ليس صورة منتشرة بالتحديد. لذلك هذه الطريقة تعطينا صورة جيدة جداً كما في الشكل 8. لكن إذا كان المدخل منتشر جداً في مجال الطول الموجي فأننا سوف نحصل صورة مثالية.       
...
حيث $M$ هي دالة قناع تصفر كل عنصر غير مختار و $\mathcal{F}$ هو عبارة عن تحويل فورير و  $y$ هي البيانات المشاهدة في مجال فورير و  $\lambda$ هي معامل قوة التنظيم و $V$ هي دالة التنظيم. 

مشكلة التحسين يجب أن تحل لكل خطوة زمن أو كل قطعة في مسح الMRI و أحياناً تأخذ زمناً أطول من المسح نفسه. و هذا يعطي السبب للبحث عن شيء أفضل.      
...
### من يحتاج إلى التحسين؟

بدلاً عن حل مشكلة التحسين الصغيرة هذه في كل خطوة زمن لماذا لا نستعمل شبكة عصبية كبيرة لإنتاج الحل المطلوب؟  أملنا هو أن ندرب شبكة عصبية بتعقيد مناسب من أجل حل مشكلة التحسين في خطوة واحدة و إنتاج مخرج جيد مثل الذي نحصل عليه من حل مشكلة التحسين بخطوات زمن. 
...
حيث $B$ هو نموذج الشبكة العصبية العميق و $y$ هي بيانات مجال فورير المشاهدة. 

قبل 15 سنة هذه الطريقة كانت صعبة لكن اﻵن أصبح تنفيذها أكثر سهولةً. شكل 9 يوضح نتائج طريقة التعلم العميق و نرى أن المخرج أفضل من مخرج طريقة الإستشعار المضغوط و يشبه جداً الصورة المأخوذة من مسح عادي. 
