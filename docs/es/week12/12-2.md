...
La búsqueda por haz es otra técnica para decodificar un modelo de lenguaje y producir texto. En cada paso, el algoritmo realiza un seguimiento de las $k$ traducciones parciales (hipótesis) más probables (mejores). La puntuación de cada hipótesis es igual a su probabilidad logarítmica.
...
¿Por qué (en NMT) los tamaños de haz muy grandes a menudo dan como resultado traslaciones vacías?
...
Resumen: Continúe la búsqueda de haces hasta que todas las $k$ hipótesis produzcan un token final o hasta que se alcance el límite máximo de decodificación T.
...
1. Pila de codificadores: la atención personal no está enmascarada, por lo que cada token en la entrada puede mirar todos los demás token en la entrada
...
- Simplificar el objetivo de preentrenamiento de BERT
