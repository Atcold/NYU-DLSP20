...
Since it is an isotropic model, the neighbours will have the same $\boldsymbol{W}^{l}$ matrix.
 -->
Se nos da el campo de recepción local por diseño, es decir, con Redes de Grafo Neuronales solo tenemos que considerar los vecinos.

Tenemos peso compartido, es decir, tenemos la misma matriz $\boldsymbol{W}^{l}$ para todas las características sin importar la posición del grafo, que es una propiedad de convolución.
