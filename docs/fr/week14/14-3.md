...
L'apprentissage par transfert (TF) consiste simplement à entraîner un classificateur final en plus d'un réseau pré-entraîné (utilisé généralement dans les cas où les données sont peu nombreuses).
...
A. Si nous avons peu de données d'entraînement.
...
- Par contraste, dans la régularisation L2, à partir du pic bleu proche de zéro, nous pouvons voir que la plupart des poids sont proches de zéro mais non nuls.

- Lorsqu'il n'y a pas de régularisation (lavande), les poids sont beaucoup plus souples et s'étalent autour de zéro, comme dans une distribution normale.
