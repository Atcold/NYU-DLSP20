...
L'idée derrière les energy based factor graphs est de construire un EBM dans lequel l'énergie est la somme de termes énergétiques partiels ou lorsque la probabilité est un produit de facteurs. L'avantage de ces modèles est que des algorithmes d'inférence efficaces peuvent être utilisés.
...
## Inférence efficace pour les Energy based factor graphs
...
Un exemple simple de graphe factoriel est présenté à la figure 19 (en haut). La fonction énergie est la somme de quatre facteurs :
...
$$(\bar{Y}, \bar{Z})=\nom de l'opérateur{argmin}_{y \in \mathcal{Y}, z \in \mathcal{Z}}\left(E_{a}\left(X, z_{1}\right)+E_{b}\gauche(X, z_{1}, z_{2}\right)+E_{c}\gauche(z_{2}, y_{1}\right)+E_{d}\gauche(y_{1}, y_{2}\right)\right)$$
...
Le graphe factoriel présenté à la figure 20 est un graphe factoriel logarithmique pour les modèles linéaires structurés ("simple energy-based factor graphs" dont nous parlons)
...
#### Question1 : Pourquoi l'inférence est-elle facile dans le cas des graphes de facteurs énergétiques ?
...
#### Question 2 : Que faire si les variables latentes des graphes factoriels sont des variables continues ? Peut-on encore utiliser l'algorithme min-sum ?
