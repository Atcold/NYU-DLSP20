...
* Progrès impressionnants au cours des dernières années :
  ...
  - Des performances supérieures à l’humaine pour de nombreux ensembles de données de réponses aux questions
...
* Enchâsse chaque mot comme un vecteur, qui est une table de recherche de la matrice d'enchâssement, de sorte que le mot obtienne le même vecteur quel que soit le contexte dans lequel il apparaît
* Appliquer le même réseau feed forward à chaque étape
* Malheureusement, la durée fixe de l'historique signifie qu'il ne peut être conditionné qu'à un contexte délimité
...
* Inconvénients :
  ...
  - Impossible à mettre en paralléliser les calcules des étapes temporelles, donc entraînement lent
...
### [Modèles linguistiques basés sur un Transformer](https://www.youtube.com/watch?v=6D4EWKJgNn0&t=828s)
...
Les sous-couches sont reliées par les cases intitulées "Add & Norm". La partie "Add" signifie qu'il s'agit d'une connexion résiduelle, qui contribue à empêcher la disparition du gradient. La "Norm" désigne ici la normalisation des couches.
...
Il convient de noter que les transformers partagent les poids entre les étapes temporelles.
...
Nous calculons la même chose avec différentes requêtes, valeurs et clés plusieurs fois en parallèle. La raison est que nous voulons prédire le mot suivant en utilisant différentes choses. Par exemple, lorsque nous prédisons le mot "licornes" en utilisant les trois mots précédents "Ces", "corne" et "blanc argenté". Nous savons que c'est une licorne par "corne" "blanc argenté". Cependant, nous pouvons savoir qu'il s'agit de "licornes" au pluriel via "Ces". Par conséquent, nous voulons probablement utiliser ces trois mots pour savoir quel sera le prochain mot. L'attention multi-têtes est une façon de laisser chaque mot regarder plusieurs mots précédents.

Un grand avantage de l'attention multi-têtes est qu'elle est très parallélisable. Contrairement aux RNNs, le calcul de toutes les têtes des modules d'attention multi-têtes et toutes les étapes de temps peuvent se faire en même temps. Un des problèmes du calcul de tous les pas de temps en une fois est qu'il pourrait également examiner les mots futurs, alors que nous ne voulons que conditionner les mots précédents. Une solution à ce problème est ce que l'on appelle le **masquage de l'auto-attention**. Le masque est une matrice triangulaire supérieure qui comporte des zéros dans le triangle inférieur et moins l'infini dans le triangle supérieur. L'effet de l'ajout de ce masque à la sortie du module d'attention est que chaque mot à gauche a un score d'attention beaucoup plus élevé que ceux à droite. Ainsi en pratique, le modèle se concentre uniquement sur les mots précédents. L'application du masque est cruciale dans le modèle linguistique car elle le rend mathématiquement correct. Cependant, dans les encodeurs de texte, le contexte bidirectionnel peut être utile.

Un détail pour faire fonctionner le transformer est d'ajouter l'enchâssement positionnel à l'entrée (l’ordre des mots dans un texte). Dans le langage, certaines propriétés comme l'ordre sont importantes à interpréter. La technique utilisée ici est l'apprentissage d’enchâssements séparés à différents moments et l'ajout de ceux-ci à l'entrée, de sorte que l'entrée est maintenant la somme du vecteur mot et du vecteur positionnel. Cela donne des informations sur l'ordre.
...
3. L'auto-attention est quadratique (tous les pas de temps peuvent s'occuper de tous les autres), ce qui limite la longueur maximale des .
...
### Astuce 2 : Échauffement (Warm-up) + grille d'entraînement
...
Voici les résultats de certaines méthodes mentionnées ci-dessus. Dans ces tests, la métrique de droite appelée `ppl` est la perplexité (plus le `ppl` est bas, mieux c'est).
...
On peut constater que lorsque les transformers ont été introduits, la performance a été grandement améliorée.
...
Nous pouvons maintenant entraîner une distribution de probabilité sur le texte. Grossièrement, nous pourrions obtenir un nombre exponentiel de sorties possibles, donc nous ne pouvons pas calculer le maximum. Le choix fait pour le premier mot peut avoir une incidence sur toutes les autres décisions.
Etant donné cela, le décodage gourmand (greedy decoding) a été introduit comme suit.
