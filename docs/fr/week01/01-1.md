...
Sur le plan conceptuel, l'apprentissage profond est inspiré par le cerveau. Cependant tous les détails du cerveau ne sont pas pertinents. À titre de comparaison, les avions ont été inspirés par les oiseaux. Le principe du vol est le même, mais les détails sont extrêmement différents.

L'histoire de l'apprentissage profond remonte à un domaine qui a changé de nom pour devenir aujourd’hui la cybernétique. Elle a commencé dans les années 1940 avec McCulloch et Pitts. Ils ont eu l'idée que les neurones sont des unités de seuil avec des états de marche et d'arrêt. Ainsi qu'il est possible de construire un circuit booléen en connectant les neurones entre eux et effectuer des inférences logiques avec les neurones. Le cerveau est essentiellement une machine d'inférence logique car les neurones sont binaires. Les neurones calculent une somme pondérée d'entrées et comparent cette somme à son seuil. Il s'allume s'il est supérieur au seuil et s'éteint s'il est inférieur, ce qui est une vue simplifiée du fonctionnement des réseaux neuronaux.

En 1947, Donald Hebb a eu l'idée que les neurones du cerveau apprennent en modifiant la force des connexions entre les neurones. C'est ce qu'on appelle l'hyper-apprentissage, où si deux neurones sont activés ensemble, la connexion entre eux augmente; s'ils ne sont pas activés ensemble, la connexion diminue.

Plus tard, en 1948, la cybernétique a été proposée par Norbert Wiener. Il se base sur l'idée qu'en ayant des systèmes avec des capteurs et des actionneurs, alors il y a une boucle de rétroaction et un système d'autorégulation. Les règles du mécanisme de rétroaction d'une voiture sont toutes issues de ce travail.
...
L'apprentissage supervisé est issu du Perceptron et de l'Adaline. L'Adaline est basé sur la même architecture avec des entrées pondérées ; quand il est au-dessus du seuil, il s'allume et en dessous du seuil, il s'éteint. Le Perceptron est un réseau de neurones à deux couches dont la deuxième couche est entraînable et la première couche est fixe. La plupart du temps, la première couche est déterminée de manière aléatoire et c'est ce qu'on appelle les couches associatives.


<!--
## [History of Pattern Recognition and introduction to Gradient Descent
](https://www.youtube.com/watch?v=0bMe_vCZo30&t=1461s)
-->
...
L'architecture multicouche la plus simple avec des paramètres réglables et la non-linéarité pourrait être la suivante : l'entrée est représentée par un vecteur tel qu'une image ou un son. Cette entrée est multipliée par la matrice de poids dont le coefficient est un paramètre accordable. Ensuite, chaque composante du vecteur de résultat est passée par une fonction non linéaire telle que ReLU (Rectified Linear Unit). En répétant ce processus, il devient un réseau neuronal de base. La raison pour laquelle il est appelé réseau de neurones est que cette architecture calcule la somme pondérée des composantes d'entrée par les lignes correspondantes d'une matrice.

Pour en revenir à l'apprentissage supervisé, nous comparons le résultat obtenu avec le résultat cible puis nous optimisons la fonction objectif qui est la perte de calcul de la distance/pénalité/divergence entre le résultat et la cible. Ensuite, nous faisons la moyenne de cette fonction de coût sur l'ensemble d’entraînement. C'est l'objectif que nous voulons minimiser. En d'autres termes, nous voulons trouver la valeur des paramètres qui minimisent cette moyenne.

La méthode pour la trouver est le calcul du gradient. Par exemple, si nous sommes perdus dans une montagne par une nuit brumeuse et que nous voulons aller au village dans la vallée. Un moyen pourrait être de faire demi-tour et de voir quel est le chemin le plus raide pour descendre, puis de faire un petit pas vers le bas. La direction est une pente (négative). En supposant que la vallée est convexe, nous pourrions atteindre la vallée.

Le chemin le plus efficace est appelé "descente stochastique du gradient" (SGD en anglais pour Stochastic Gradient Descent). Comme nous voulons minimiser la perte moyenne sur l'ensemble d’entraînement, nous prenons un échantillon ou un petit groupe d'échantillons et nous calculons l'erreur, puis nous utilisons la descente du gradient. Ensuite, nous prenons un nouvel échantillon et obtenons une nouvelle valeur pour l'erreur, puis nous obtenons le gradient qui est une direction différente normalement. Deux des principales raisons d'utiliser la SGD sont qu'il permet à un modèle de converger rapidement de manière empirique si l'ensemble d’entraînement est très grand, et qu'il permet une meilleure généralisation ce qui signifie obtenir des performances similaires sur différents ensembles de données.
...
Notez qu'au lieu d'entrées scalaires, il s'agira d'entrées vectorielles. Plus généralement, il s'agit d'entrées multidimensionnelles. La rétropropagation permet de calculer la dérivée de la différence entre la sortie souhaitée et la sortie obtenue (qui est la valeur de la fonction objectif) par rapport à toute valeur à l'intérieur du réseau. Enfin, la rétropropagation est essentielle car elle s'applique à plusieurs couches.

Il est important de réfléchir à la manière d'interpréter les entrées. Par exemple, une image de $256$\times$256$ nécessiterait une matrice d'une valeur de 200 000. Il s'agirait d'énormes matrices que les couches du réseau neuronal devraient manipuler. Il ne serait pas pratique d'utiliser de telles matrices. Par conséquent, il est important de faire des hypothèses sur la structure de la matrice.
