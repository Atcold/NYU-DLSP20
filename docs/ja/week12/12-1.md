...
1. 必要とするより遥かに膨大な無制限の訓練データ
2. GPT 2は2019年に20億個のパラメータを使用しました。
3. 最近（2020年）のモデルでは170億個までのパラメータを使用しました。
<!-- TODO: typo -->
...
1. single-headのattentionモデルとは対照的に、multi-headのattentionモデルの利点は何ですか?
...
3. TransformerはGPU並列化を利用する点でRNNとどのように違うのでしょうか？
