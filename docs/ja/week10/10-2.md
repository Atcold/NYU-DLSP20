...
<!-- TODO: Could not understand -->
<!-- word course 語源？ -->
語源になっています。ここに挿入してるかなり人気のある手作りの特徴であるSIFTのようなものは、不変になります。そして教師ありネットワーク、例えば教師ありAlex netは、Data Augmentation（データ拡張）に対して不変になるように訓練されています。このネットワークには、入力に適用された変換が正確に何であったかを予測するように求めるのではなく、この画像の異なるクロップ（トリミングのようなもの）や異なる回転を木として分類するようにして欲しいと考えます。
...
3. どのような不変性が重要なのか？(特定の教師ありタスクに対してどのような不変性が働くかについては、今後の課題として一般的に考えることができます。)
