...
1. 重み付けされた和を積み重ねた層よりも複雑な計算で推論が行われる場合はどうするか？
...
<!--  ## Latent-Variable EBM -->
潜在変数を導入することのもう一つの大きな利点は、潜在変数をある集合上で変化させることによって、予測出力$y$も、ありうる予測の集まりからなる多様体上で変化させることができることです（ひも（で表された多様体は）下の図に示されています）。: $F(x,y) = \text{argmin}_{z} E(x,y,z)$
