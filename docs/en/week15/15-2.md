...
Let's take a closer look at $\vect{y}_1=0$, with the following cold-warm map:
...
If we take $\vect{y}_2=0.4$, then $F_\beta(\vect{y})=0$ and as we move linearly away from the point towards the right-hand side, the free energy increases quadratically. Similarly, if we move towards 0, you will end up climbing towards the parabola, creating a peak in the center.
...
$$F_\beta(\vect{y})\dot{=}-\frac{1}{\beta} \log \frac{1}{|\mathcal{Z}|}{\int}_\mathcal{Z} \exp[{-\beta}E(\vect{y},z)]\mathrm{d}z$$
...
$$\tilde{F}_\beta(\vect{y})=-\frac{1}{\beta} \log \frac{1}{|\mathcal{Z}|}\underset{z\in\mathcal{Z}}{\sum} \exp[{-\beta}E(y,z)]\Delta z$$
Here, we define $-\frac{1}{\beta} \log \frac{1}{|\mathcal{Z}|}\underset{z\in\mathcal{Z}}{\sum} \exp[{-\beta}E(\vect{y},z)]$ to be the $\smash{\underset{z}{\text{softmin}}}_\beta[E(\vect{y},z)]$, such that the relaxation of the zero temperature limit free energy becomes the *actual*-softmin.
**Examples:**
...
Formally, we define $\underline{\texttt{actual Softmax}}$  as

$$\smash{\underset{z}{\text{softmax}}}_\beta[E(y,z)] \dot{=} \frac{1}{\beta} \log \underset{z\in\mathcal{Z}}{\sum} \exp[{\beta}E(\vect{y},z)] - \frac{1}{\beta} \log{|\mathcal{Z}|}$$

To implement the above function in PyTorch, use "torch.logsumexp" as below:

$$\smash{\underset{z}{\text{softmax}}}_\beta[E(y,z)] \dot{=} \frac{1}{\beta} \texttt{torch.logsumexp}({\beta}E(\vect{y},z),dim=z)- \frac{1}{\beta} \log{|\mathcal{Z}|}$$


$\underline {Actual-Softmin}$

$$\smash{\underset{z}{\text{softmin}}}_\beta[E(y,z)] \dot{=} -\frac{1}{\beta}\log\frac{1}{|\mathcal{Z}|}\underset{z\in\mathcal{Z}}{\sum}\exp[-{\beta}E(\vect{y},z)]$$

Softmin can be implemented using softmax with 2 negative signs as follows:

$$\smash{\underset{z}{\text{softmin}}}_\beta[E(y,z)]\dot{=} -\smash{\underset{z}{\text{softmax}}}_\beta[E(y,z)]$$


$$\texttt{torch.softmax}(l(j),dim=j) = \underset{j}{\texttt{softargmax}_{\beta=1}}[l(j)]$$
...
$$l_{\text{energy}}(F(\cdot),\check{\vect{y}}) = F(\check{\vect{y}}) $$ where $l_{energy}$ is the energy loss functional evaluated in $\vect{y}$ and $\vect{y}$ is the datapoint on the dataset. $\check{\vect{y}}$ indicates the datapoints that are to be pushed down. This means the loss functionals should be minimized while training.

The Energy function should be small for data that comes from training distribution but large elsewhere.


$$l_{\text{hinge}}(F(\cdot),\check{\vect{y}},\hat{\vect{y}}) = [m+F(\check{\vect{y}})-F(\hat{\vect{y}})]^{+}$$

where m is the margin and $F(\check{\vect{y}})$ and $F(\hat{\vect{y}})$ are the free energies for cold(energies for correct labels) and hot temperatures(energies for incorrect labels)respectively.

The model tries to make the difference between two energies to be larger than the margin m.

Understand that there is a ReLu function $[\cdot]^{+}$ used on the output of $[m+F(\check{\vect{y}})-F(\hat{\vect{y}})]$, which means the value of this hinge loss function will always be in the range of max(0,.). This implies if there are any negative values, they would become 0 due to this function.

Hence, $F(\check{\vect{y}})-F(\hat{\vect{y}})$ should be less than m. If the difference becomes greater than m, the overall value of $[m+F(\check{\vect{y}})-F(\hat{\vect{y}})]$ becomes negative, the hinge loss becomes 0. We can also say that we push the energies as long as the difference is less than m. However, if the differene becomes greater than the margin m, we stop pushing. Hinge Loss Function does not have a smooth margin.
...
$$F_{\beta}(\vect{y}) \dot{=} -\frac{1}{\beta}\log\frac{1}{|\mathcal{Z}|}\int_\mathcal{Z}\exp[-{\beta}E(\vect{y},z)]\mathrm{d}z$$
...
z takes values linearly, and is feeded into the decoder to obtain $\tilde{\vect{y}}$, which will go around the ecllipse.
$$x = [0:\frac{1}{50} :1]$$
...
$$
In this case the $g$ fucntion takes $f$ and $z$, and $g$ can be a neural net. This time, the model has to learn that $\vect{y}$ moves around in circle, which is the $\sin$ and $\cos$ part we take for granted before.
