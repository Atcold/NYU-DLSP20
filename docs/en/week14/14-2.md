...
<center>
<img src="{{site.baseurl}}/images/week14/14-2/Table_loss.png" style="background-color:#DCDCDC;" /><br>
...
<b>Figure 3</b>: Graph Composition
</center>
...
<center>
<img src="{{site.baseurl}}/images/week14/14-2/check_reader.png" height="50%" width="50%" /><br>
<b>Figure 4</b>: Check reader
</center>
...
$$F_{\beta} (x, y) = -\frac{1}{\beta} \; log \; \Sigma_{z \, \in \, paths} \; \exp \, (- \beta \; E(x, y, z))$$
...
<b>Figure 5</b>: Interpretation graph
</center>
...
$$ \alpha_{i} = - \; log \; \Sigma_{k \, \in \, parent \, (i)} \; \exp \, (- \, \beta \; (\alpha_k \, + \, e_{ki}))$$
...
$$L(x,y) = -\frac{1}{\beta}\int_z e^{-\beta E(x,y,z)}$$

If we then multiply by $\frac{q(z)}{q(z)}$, we get
$$L(x,y) = -\frac{1}{\beta}\int_z q(z) \frac{e^{-\beta E(x,y,z)}}{q(z)}$$

If we assume that $q(z)$ is a probability distribution over $z$, we can interpret our rewritten loss function integral as an expected value, with respect to the distribution of $\frac{e^{-\beta E(x,y,z)}}{q(z)}$.
...
<b>Figure 1:</b> Jensen's Inequality (taken from https://en.wikipedia.org/wiki/Jensen%27s_inequality)
</center>
...
$$F(\int_z q(z)h(z)) \leq \int_z q(z)F(h(z)) \tag{1}$$

Now, recall that our marginalized $L(x,y)$ after multiplication with $\frac{q(z)}{q(z}$ is,  
$$L(x,y) = -\frac{1}{\beta}\int_z q(z) \frac{e^{-\beta E(x,y,z)}}{q(z)}$$

If we make $h(z) = -\frac{1}{\beta} \frac{e^{-\beta E(x,y,z)}}{q(z)}$, we know from Jensen's Inequality $(1)$ that

$$F(\int_z q(z)\frac{e^{-\beta E(x,y,z)}}{q(z)}) \leq \int_z q(z)F(\frac{e^{-\beta E(x,y,z)}}{q(z)})$$

Let's continue to work this, with a concrete convex loss function, $F(x) = -log(x)$

$$-log(-\frac{1}{\beta}\int_z q(z)\frac{e^{-\beta E(x,y,z)}}{q(z)}) \leq \int_z q(z) * \frac{-1}{\beta}log(\frac{e^{-\beta E(x,y,z)}}{q(z)})$$

$$\leq \int_z q(z)[E(x,y,z) + \frac{1}{\beta}log(q(z))]$$

$$\leq \int_z q(z)E(x,y,z) + \frac{1}{\beta}\int_z q(z)log(q(z))$$

Great! Now we have an upper bound to our loss function $L(x,y)$, composed of two terms we understand. The first term $\int_z q(z)E(x,y,z)$ is the *average* energy. And the second term $\frac{1}{\beta}\int_zlog(q(z))$ is just some factor ($-\frac{1}{\beta}$) times the *entropy* of the distribution $q$.
