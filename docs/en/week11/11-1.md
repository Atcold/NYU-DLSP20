...
<center>
<img src="{{site.baseurl}}/images/week11/11-1/RReLU.png" height="400px" /><br>
...
The bottom part of the equation prevents the problem of dying ReLU which refers to the problem when ReLU neurons become inactive and only output 0 for any input. Therefore, its gradient is 0. By using a negative slope, it allows the network to propagate back and learn something useful.
...
![Hardshrink](media/week11/Hardshrink.png)
<center>
...
![LogSigmoid](media/week11/LogSigmoid.png)
<center>
...
- Differences between a nonliner activation having kinks and a smooth nonlinear activation. Why/when one of them is preferred?
...
Mathematically, $H(p,q) = H(p) + \mathcal{D}_{KL} (p || q)$, where $H(p,q) = - \sum_i p(x_i) \log (q(x_i))$ is the cross-entropy (between two distribution), $H(p) = - \sum_i p(x_i) \log (p(x_i))$ is the entropy, and $\mathcal{D}_{KL} (p || q) = \sum_i p(x_i) \log \frac{p(x_i)}{q(x_i)}$ is the KL divergence.
