...
This method will limit the volume of space of z which takes a small value and the value which will, in turn, controls the space of y that has low energy. The value of Lambda controls this trade-off. A useful example of $R$ is the $L_1$ norm, which can be viewed as an almost everywhere differentiable approximation of effective dimension. Adding noise to $z$ while limiting its $L_2$ norm can also limit its information content (VAE).
