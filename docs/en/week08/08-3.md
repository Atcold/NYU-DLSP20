...
- First, the autoencoder takes in an input and maps it to a hidden state through an affine transformation $\boldsymbol{h} = \sigma(\boldsymbol{W}_h \boldsymbol{x} + \boldsymbol{b}_h)$, where $\sigma$ is an (element-wise) activation function. This is the **encoder** stage. Note that $\boldsymbol{h}$ is also called the **code**. 
- Next, $\hat{\boldsymbol{x}} = \sigma'(\boldsymbol{W}_x \boldsymbol{h} + \boldsymbol{b}_x)$. This is the **decoder** stage. 
...
In fact, for classic autoencoder, we can think of $\boldsymbol{h}$ as just the vector $\boldsymbol{E}(\boldsymbol{z})$ in the VAE formulation. In short, the main difference between VAEs and AEs is that VAEs have a good latent space that enables generative process. 
...
$$
l(\boldsymbol{x}, \hat{\boldsymbol{x}}) = \frac{1}{2} ||\boldsymbol{x} - \hat{\boldsymbol{x}} ||^2
$$
...
During training, the `reparameterise` function will return a different $z$ even if we have the same $\mu$ and same $\text{std}$ because we have different eps ($\boldsymbol{\epsilon}$). To connect to the concept of a yellow bubble, as explained in the lecture, the line `eps.mul(std).add_(mu)` compute a new points from $N(\mu,std)$. So if we do 100 times, we will have a yellow bubble with 100 points, filling up its volume.
