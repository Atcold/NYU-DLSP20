...
$$
    \boldsymbol{h}=\boldsymbol{x} + \bigg(\boldsymbol{Ax} + \sum_{v_j→v}{\eta(\boldsymbol{e_{j}^{h}})\odot \boldsymbol{Bx_{j}}}\bigg)^{+}
\tag{Eq. 5}
$$
...
$$
    \eta(\boldsymbol{e_{j}^{h}})=\sigma(\boldsymbol{e_{j}^{h}})\bigg(\sum_{v_k→v}\sigma(\boldsymbol{e_{k}^{h}})\bigg)^{-1}
\tag{Eq. 6}
$$
