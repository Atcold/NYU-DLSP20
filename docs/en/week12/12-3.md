...
$$
There is also the concept of "heads". The above could represent one head, but we can multiple heads. For example, say we have $h$ heads, we get a vector in $\mathbb{R}^{3hd}$, i.e we have $h$ $\vect{q}$'s, $h$ $\vect{k}$'s and $h$ $\vect{v}$'s:
$$
\begin{bmatrix}
\vect{q}^1 \\
\vect{q}^2 \\
...
However, we can still transform this to have the original dimension $R_{d}$ by using a $\vect{W_h} \in \mathbb{R}^{d \times hd}$. This is just one possible way to implement the key-value store.
