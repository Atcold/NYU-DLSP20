...
university of washington and working with luke zettelmoyer
...
okay well thank you very much for the introduction of [inaudible] and inviting me
...
translations which [inaudible] human narrators prefer the ones
...
how all [inaudible] here is in this context encoder
...
that word was [inaudible] um one final issue i want to mention
...
experiment with using things like really use instead
...
the stuff like um multi-headed attention and [inaudible] embeddings and stuff so
...
uh 48 um you see that Yann Lecun's convolutional
...
uh there's no [inaudible] programming here so
...
example um this slides from abigail c at stanford um
...
it's any good or not then uh this is not real and
...
so what we're trying these models typically what we do is we rely on labeled
...
okay um so but it worked very well um they gave
...
but um burt was definitely not the end of the story here um
...
facebook uh yeah by led by Y Liu which looked at scaling up bert um
...
exactly one-on-one and not for i mean it's just because everyone knows that um
...
graph neural nets from the [inaudible] actually about graph neural net and
