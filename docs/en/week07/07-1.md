...
$$
\hat{y} = \displaystyle \text{argmin}_y \left \{ F(x,y)\right \}
$$
...
$$\hat{y}, \hat{z} = \text{argmin}_{y,z} E(x,y,z)$$
 ...
 And this is equivalent to redefining the energy function as: $F_\infty(x,y) = \text{argmin}_{z}E(x,y,z)$,
 
 which equals to: $F_\beta(x,y) = -\frac{1}{\beta}log\int_z e^{-\beta E(x,y,z)}$. When $\beta \rightarrow \infty$, then $\hat{y} =\text{argmin}_{y}F(x,y)$.
 
 
<!--  ## Latent-Variable EBM -->
 There is also a big advantage of allowing latent variables. By varying the latent variable over a set, we can make the prediction output y varies over the manifold of possible predictions as well (the ribbon is shown in the graph below): $F(x,y) = \text{argmin}_{z} E(x,y,z)$. 
...
One example is video prediction. There are many reasons for us to do video prediction, one example is to make a video compression system. Another good reason is the video we are trying to predict is the video you are looking at from the windshield when you are driving a car and you want to predict what the cars around you would be going to do. And this is also the essence of artificial intelligence.

Another example of this is translation. Language translation is always a big problem because there is no single correct translation for a piece of text from one language to another. Usually, there are a lot of different ways to express the same idea and people find it is hard to reason why they pick one over the other. So it might be nice if we have some way of parameterizing all the possible translations that a system could produce to respond to a given text. Let's say if we want to translate German to English, there could be multiple translations in English that are all correct, and by varying some latent variables then you may vary the translation produced. 
...
$$P(y \mid x) = \frac{e^{-\beta F(x,y)}}{\int_{y'}e^{-\beta F(x,y')}}$$ 
...
$$P(y,z \mid x) = \frac{e^{-\beta F(x,y,z)}}{\int_{y}\int_{z}e^{-\beta F(x,y,z)}}$$

Now if marginalize over y: $P(y \mid x) = \int_z P(y,z \mid x)$, we have:

<center>
<img src="{{site.baseurl}}/images/week07/07-1/fig2.png"/><br>
<b>Fig. 2</b>: Derivation for marginalization in probabilistic models
...
Actually computing this can be very hard...  In fact, in most cases, it's probably intractable.  So, the last few slides were just to say, if you have a latent variable that you want to minimize over inside of your model, or if you have a latent variable that you want to marginalize over (which you do by defining this Energy function $F$), and minimizing corresponds to the infinite *β* limit of this formula, then it can be done.

Under the definition of $F_\beta(x, y)$ above, $P(y \mid x)$ is just an application of the Gibbs-Boltzman formula.  And $z$ has been marginalized implicitly inside of this.  Physicists call this "Free Energy" by the way, which is why we call it $F$.   So $e$ is the energy, and $F$ is free energy.
