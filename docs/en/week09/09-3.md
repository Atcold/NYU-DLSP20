...
To ensure that high cost is assigned to points outside the data manifold and low cost is assigned to points within it, the loss function for the cost nework $\mathcal{L}_{C}$ is $C(x)+[m-C(G(z))]^+$ for some positive margin $m$. Minimizing $\mathcal{L}_{C} requires that $C(x) \rightarrow 0$ and $C(x) \rightarrow m$. The loss for the generator $L_{G} is simply $C(G(z))$, which encourages the generator to ensure that $C(G(z)) \rightarrow 0$. However, this does create instability as $0 \leftarrow C(G(z)) \rightarrow m$.
