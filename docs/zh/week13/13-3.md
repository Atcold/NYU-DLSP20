...
在自我注意力机制中，In self-attention，we have a set of input $\lbrace\boldsymbol{x}\_{i}\rbrace^{t}_{i=1}$，我们有一个输入集 $\lbrace\boldsymbol{x}\_{i}\rbrace^{t}_{i=1}$ 。不像序列那样，它没有顺序的。
