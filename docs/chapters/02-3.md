...
4. `J.backward()` does back-propagation and accumulation: It computes $\frac {\partial \, loss}{\partial \, x}$ for every parameter x for which we have specified `requires_grad=True`. These are accumulated into the gradient of each variable: $x.grad = x.grad +  \frac {\partial \, loss}{\partial \, x}$

5. `optimiser.step()` takes a step in gradient descent: $\theta = \theta - \alpha \; \frac {\partial \, J}{\partial \, \theta}$
