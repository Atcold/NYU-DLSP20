...
```python
x = torch.rand(1, 20, 64, 128)
conv = nn.Conv2d(20, 16, (3, 5))
...
Also, instead of basic numerical operations, we can generate our own self-defined modules/functions, which can be plugged into the nerual graph. (see *notebook -- extra -- b-custom_grads.ipynb*)
