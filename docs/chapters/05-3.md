...
<center> 
<img src="../05-3/Illustration_1D_Conv.png" alt="1" style="zoom:40%;" /><br>
...
<center> 
<img src="../05-3/Result_1D_Conv.png" alt="2" style="zoom:40%;" /><br>
...
<center>
<img src="../05-3/Monophonic_Signal.png" alt="3" style="zoom:40%;" /><br>
...
<center>
<img src="../05-3/Layer_by_layer_scalar_product.png" alt="4" style="zoom:40%;" /><br>
...
```python
conv = nn.Conv1d(2, 16, 3)  # 2 channels (stereophonic signal), 16 kernels, kernel size of 3
conv.weight.size()  # output: torch.Size([16, 2, 3])

x = torch.rand(1, 2, 64)  # the input signal has a batch of size 1, 2 channels, 64 samples
conv.bias.size()  # output: torch.Size([16])
conv(x).size()  # output: torch.Size([1, 16, 62])

conv = nn.Conv1d(2, 16, 5) # 2 channels, 16 kernels, kernel size of 5
...
```python
conv = nn.Conv2d(20, 16, (3, 5), 1, (1, 2))  # 20 channels, 16 kernels, kernel size is 3 x 5, stride is 1, add padding=(1,2) which explaned above
...
<center>
<img src="../05-3/Flow_Chart.png" alt="5" style="zoom:60%;" /><br>
...
$$
\begin{aligned}
&a = \frac{1}{4}\sum_{i=1}^{4}z_i\\[2mm]
&z_i = 3y_i^2 = 3(x_i-2)^2\\[2mm]
&\frac{da}{dx_i} = \frac{1}{4}\times3\times2(x_i-2) = \frac{3}{2}x_i-3\\[2mm]
&x = \begin{bmatrix} 1&2\\3&4\end{bmatrix}
\\[2mm] &(\frac{da}{dx_i})^{T} = \begin{bmatrix} 1.5-3&3-3\\[2mm]4.5-3&6-3\end{bmatrix}=\begin{bmatrix} -1.5&0\\[2mm]1.5&3\end{bmatrix}
\end{aligned}
$$
...
Now we have a $1\times3$ vector $x$, assign $y$ to the double $x$ and keep doubling $y$ until its norm is smaller than $1000$. Due to the randomness we have for $x$, we can not directly know the number of iterations when the procedure terminates.
...
## More Stuff -- Custom Grads

Also, instead of basic numerical operations, we can generate our own self-defined modules/functions, which can be plugged into the nerual graph. The Jupyter Notebook can be found [here](https://github.com/Atcold/pytorch-Deep-Learning/blob/master/extra/b-custom_grads.ipynb)

To do so, we need to inherit `torch.autograd.Function` and override `forward()` and `backward()` functions. For example, if we want to training nets, we need to get the forward pass and know the partial derivatives of the input respect to the output, such that we can use this module in any kind of point in the code. Then, by using back-propogation (chain rule), we can plug the thing anywhere in the chain of operations, as long as we know the partial derivatives of the input respect to the output.
...
If we have addition of two things and get an output, we need to overwrite the forward function like this. And when we go down to do back propogation, the gradients copied over both sides. So we overwrite the backward function by copying.

For `split` and `max`, see the code of how we overwrite forward and backward functions in *notebook*. If we come from the same thing and **Split**, when go down doing gradients, we should add/sum them. For **ArgMax**, it selects the index of the highest thing, so the index of the highest should be $1$ while others being $0$. Remember, according to different custom modules, we need to overwrite its own forward pass and how they do gradients in backward function.