...
<img src="02-2/02-2-1.png" alt="Gradient diagram" style="zoom:40%;" />
...
- Linear: $Y=W\cdot X$ 
  $$
  \frac{dC}{dX}= W^T \cdot \frac{dC}{dY}\\
   \frac{dC}{dW}= \frac{dC}{dY} \cdot X^T
  $$
...
- Max: $Y=\texttt{max}(X_1,X_2)$

  -  Since this function can also be represented as
    $$
    Y=\texttt{max}(X_1,X_2)=\begin{cases} 
          X_1 & X_1 > X_2 \\
          0 & else
       \end{cases}
    $$
...
![Sigmoid function to illustrate vanishing gradient](02-2/02-2-2.png)
...
![Plot of logarithmic part of the functions](02-2/02-2-3.png)
...
$$
L1 regularization (Lasso) is similar, except we use $\sum_i |w_i|$ instead of $||w||^2$.
