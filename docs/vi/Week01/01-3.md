...
Ma trận đầu tiên ánh xạ đầu vào hai chiều vào một lớp ẩn trung gian 100 chiều. Sau đó, chúng ta có một lớp phi tuyến tính, ReLU hoặc Đơn vị tuyến tính đã chỉnh lưu, chỉ đơn giản là phần dương $ (\ cdot) ^ + $function. Tiếp theo, để hiển thị hình ảnh của chúng tôi dưới dạng biểu diễn đồ họa, chúng tôi bao gồm một lớp nhúng ánh xạ đầu vào của lớp ẩn 100 chiều với đầu ra hai chiều. Cuối cùng, lớp nhúng được chiếu tới lớp năm chiều cuối cùng của mạng, biểu thị điểm cho mỗi màu.
...
Trong eq. (1), các ma trận $U$ and $V^\top$ là trực giao và biểu diễn các phép biến đổi quay và phạn xạ. Ma trận giữa là đường chéo và đại diện cho một phép biến đổi tỷ lệ.
