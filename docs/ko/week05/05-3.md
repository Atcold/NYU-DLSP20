...
$\Omega$는 $\mathbb{N}^1$안에 포함된 부분 공간이고(이는 1차원 신호이기 때문에, 1차원 도메인을 가진다.) 이 때, 채널 수 $c$는 $1$이며 $c = 2$일 때, 스테레오 신호가 된다.
...
팁: 우리는 I파이썬에서 물음표를 이용하여 각 기능들에 대한 설명을 확인할 수 있다. 예를 들면, 다음과 같다.
...
만약 차원수를 동일하게 맞추기를 원한다면, 패딩을 이용하면 해결할 수 있다. 위 코드에 이어 합성곱 함수에 새로운 매개 변수 값을 추가할 수 있다.: `stride=1`과 `padding=(1,2)`인데, 이는 $y$축 방향(위로 $1$, 아래로 $1$)으로 $1$을 의미하고, $x$축 방향으로 $2$를 의미한다. 그러면 출력 신호는 입력 신호와 비교하여 동일한 크기에 있게 된다. 2D 합성곱을 실행할 때, 커널수를 모아 저장해야하는 차원수는 $4$이다.
...
파이토치에서 편도함수를 이용할 때마다, 원래 데이터의 동일한 형태를 얻는다. 하지만 정확한 야코비안 방식은 다른 방식일 것이다.
...
지금 우리에게 $1\times3$ 벡터 $x$가 있고, $y$를 $x$의 2배만큼 부여하는데 이때 $y$의 놈이 $1000$보다 작아질 때까지 계속 2배씩 부여한다. $x$에 대하여 우리가 가진 무작위성 때문에, 연산 과정이 끝날 때 반복 횟수를 바로 알 수 없다.
...
그러나, 우리가 가진 경사를 알고 있기 때문에 쉽게 추론할 수 있다.
...
추론에 대하여 살펴보면, 아래에 코드에 나와있듯이 우리가 원하는 경사 축적을 따라가기 위한 자취를 남기기 위하여 `requires_grad=True`를 사용할 수 있다. 만약 $x$나 $w$ 선언에서 `requires_grad=True`를 빠뜨리고 $z$상의 `backward()`를 호출하면, $x$sk $w$상의 경사 축적이 없기 때문에 런타임 에러가 발생하게 된다.
...
이를 위하여 `torch.autograd.Function`을 상속받아야 하고 `forward()`와 `backward()` 함수를 무시해야 한다. 예를 들어, 신경망을 학습시키고자 할 때, 순방향으로 진행되도록 해야하고 코드에서 어느 위치에서든 사용할 수 있는 모듈과 같은 출력값에 대한 입력 의 편도함수를 알아야 한다. 그리고 역전파(연쇄규칙)을 통해 출력값에 대한 편도함수를 알기 때문에 연속적인 연산이 일어나는 어느 지점에서든지 이를 연결할 수 있다.
...
우리가 두 가지 추가 사항을 가지고 출력 결과를 얻는다면, 이와 같은 순방향 함수에 덮어써야 한다. 그리고 역전파 단계로 내려갈 때, 경사는 양쪽 모두를 복사한다. 그래서 복사에 의한 역방향 함수를 덮어써야 한다. 
...
`split`과 `max`에 대해서는 *notebook*에서 순방향, 역방향 함수를 덮어씌운 코드를 살펴볼 것을 권한다. **Split**과 동일한 것에서 출발한다면, 경사를 구하는 단계로 내려갈 때, 우리는 그것들을 추가하거나 더해야 한다. `argmax`의 경우, 가장 높은 것의 인덱스 값을 선택하므로, 가장 높은 것의 인덱스 값은 반드시 $1$이고 아닌 것은 $0$이어야 한다. 다른 맞춤 모듈에 따라 우리가 기억해야 할 것은 순방향으로 진행될 수 있도록 덮어씌워야 한다는 것과 역방향 함수에서 경사를 어떻게 하는지 여부이다.