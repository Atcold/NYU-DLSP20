...
## 전통적인 합성망<sup>Traditional ConvNets
</sup>

합성망은 고차원 학습 문제에 매우 효과적인 구조이다.  
...
1024 x 1024 픽셀을 가진 이미지는 1,000,000 차원 공간의 한 점으로 생각될 수 있다. 차원별로 10개의 샘플들을 사용한다면 ${10}^{1,000,000}$ 라는 엄청난 수의 이미지들을 생성한다. 합성망은 다음 예제에서 보여지는 것과 같이 고차원 이미지 데이터 최적의 표현을 추출하는 데 매우 효과적이다. 
...
<center>
<img src="{{site.baseurl}}/images/week13/13-1/ConvNets.png" height="100%" width="100%"/><br>
<b>Fig. 1</b>: 합성망은 고차원 이미지 데이터에서 표현을 추출한다.
</center>
...
-->
* **집약적<sup>Local</sup>** 신경망에서의 뉴런은 신경망 전체의 계층이 아닌 오직 인접한 계층들하고만 연결되어있다. 이를 집약적 수용영역 가정<sup></sup>이라고 부른다. 
* **정상적<sup>Stationary</sup>** 비슷한 패턴들을 가지며 이미지 영역<sup></sup>에 걸쳐 공유한다. 예로 figure 2에서의 중간 이미지의 노란 침대 시트를 들 수 있다.
* **계층적<sup>Hierarchical</sup>** 저레벨 특징들은 모여서 중간레벨 특징을 이루고, 이 중간레벨 특징들이 계속해서 모여 더욱더 높은 레벨의 특징<sup></sup>들을 이룬다. 예로 시각표현을 들 수 있다. 
<!--
<center>
...
* 이미지, 부피, 비디오는 각각 2D, 3D, 2D + 1 유클리드<sup>Euclidean</sup>의 정의역에 펼쳐져 있으며, 각 픽셀은 빨강, 초록, 파랑값의 벡터에 의해 표현된다. 
* 
<center>
<img src="{{site.baseurl}}/images/week13/13-1/Image.png" height="80%" width="80%"/><br>
...
<center>
  <img src="{{site.baseurl}}/images/week13/13-1/Sequence.png" height="80%" width="80%"/><br>
  <b>Fig. 5</b>: 순차<sup></sup>는 하나의 차원을 가진다.
  </center>
...
## 전통적 합성망의 합성곱<sup>Convolution</sup>
...
템플릿 매칭은 공간적 그래프 합성망에, 그리고 합성곱 이론은 스펙트럼 합성망에 사용될 것이다. 
