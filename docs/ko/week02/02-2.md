...
우리는 역전파에 대한 구체적인 예시를 그래프 시각자료를 통하여 살펴본다. 임의의 함수 $G(w)$는 손실 함수 $S$의 입력으로 사용된다. 이때 손실함수 역시 그래프로 표현 가능하다. 야코비안 행렬(Jacobian matrics)의 행렬 곱 연산을 통하여, 그래프로 표현된 손실함수는 그 기울기를 역전시킬수 있는 그래프로 변환할 수 있다. (Pytorch와 TensorFlow는 위 파이썬 라이브러리 사용자들을 위해서 위 작업들의 자동화를 지원한다. 예를 들어서 기울기를 역전파하는 미분 그래프를 생성하기 위해서 순전파 그래프는 자동으로 역전된다.)
...
그래프의 구조가 고정되어있지 않더라도, 데이터에 따라서 복잡성이 발생할 수 있다. 예를 들어 입력 벡터의 길이에 따라서 우리는 신경망 모듈을 선택할 수 있다. 루프의 수가 연산 가능한 양을 초과하게 된다면, 관리가 점점 더 어려워지는 문제가 발생할 수 있다. 
...
익숙한 선형 모듈과 ReLU 모듈 이외에도 다양한 유형의 빌드 된 모듈이 있다. 이들은 각각의 기능을 수행하기 위해 고유하게 최적화되어 있기 때문에 편리하다. (다른 기본 모듈의 조합에 의해 구축되는 것이 아니다)
...
- Duplicate: $Y_1=X$, $Y_2=X$
  ...
  - 두 출력이 입력과 동일한 'Y- 스플리터'과 비슷합니다.

  <!-- - When backpropagating, the gradients get summed -->
  - 역전파가 진행되면서, 기울기가 모두 더해집니다.
...
- Add: $Y=X_1+X_2$
  ...
  - 두 변수가 합산됨에 따라서, 하나의 변수가 변동함에 따라서, 다른 변수도 동일한 양으로 변동하게 될 것입니다. 아래의 수식을 통해 예시를 확인 할수 있습니다.
...
Pytorch 모듈이기도 한 SoftMax는 일정한 시퀀스의 숫자들을 0과 1 사이의 양의 수로 변환하는 방법을 뜻한다. 이러한 숫자는 확률 분포로 해석될 수 있다. 이러한 성질로 인하여, 분류 문제에서 일반적으로 사용된다. 아래 방정식에서 $ y_i $는 모든 카테고리에 대한 확률의 벡터이다.
...
그러나 softmax를 사용하면 신경망은 기울기 손실<sup>vanishing gradients</sup>에 취약해진다. 기울기 손실 문제는 학습이 진행됨에 따라서, 손실함수의 기울기가 인공 신경망의 각 레이어에 전달 되면서 신경망의 가중치를 학습시키는 걸 막는다. 예를 들어, 단일 값에 대한 softmax 함수 인 로지스틱 시그모이드 함수는 s가 크면 $h(s)$가 1이고 s가 작을 때 $h(s)$가 0임을 나타낸다. $h(s)=0$ 및 $h(s)=1$에서 평평한 경우 기울기는 0이므로 그라디언트가 사라지게 된다.
...
ReLU는 여러 계층의 네트워크에서 가장 잘 작동하는 비선형 함수입니다. ReLU의 대안으로 사용되는 시그모이드 함수 및  $\tanh(\cdot) $ 함수와 같은 비선형 함수보다 선호 됩니다. ReLU가 가장 잘 작동하는 이유는 하나의 꺽인점<sub>kink</sub>만 사용해서 등가 스케일링이 가능하게 됩니다.
<!-- 
### Use cross-entropy loss as the objective function for classification problems -->

### 분류 문제 있어서 cross-entropy loss를 목적 함수로 사용한다.
...
앞에서 설명한 것처럼 데이터에 중복성이 있기 때문에 미니 배치를 통해서 효율적으로 훈련이 가능하다. 기울기를 연산하기 위해 모든 단계에서 모든 관측치에서 예측을하고 손실을 계산할 필요는 없다. (배치 단위로 연산된다.)
...
학습에서 순서는 중요하다. 만약 모델이 각 학습 단계에서 단일 클래스의 예제만 보는 경우 해당 클래스를 예측해야하는 이유를 배우지 않고 해당 클래스를 예측하는 방법을 학습한다. 예를 들어 섞이지 않은 MNIST dataset의 digits을 학습하는 경우에 있어서, 마지막 레이어의 Bias 파라미터는 항상 0으로 예측된다. 이러한 경우, 항상 1이나 2가 되도록 분류기는 적응한다. 이상적으로는 모든 Minibatch마다 훈련 데이터를 섞는 것이 중요하다.

그러나 모든 Epoch 에서 훈련데이터의 순서를 변경해야하는지에 대한 논의가 진행 중이다.

<!-- ### Normalize the inputs to have zero mean and unit variance -->
### 평균값과 단위 분산이 0이되도록 입력을 정규화
...
학습하기 전에 각 입력 기능을 정규화하여 평균이 0이고 표준 편차가 1이되도록 하는것은 유용하다. RGB 이미지 데이터를 사용하는 경우 각 채널의 평균 및 표준 편차를 개별적으로 취하여 이미지를 채널 단위로 정규화하는 것이 일반적이다. 예를 들어 데이터 세트에있는 모든 파란색 값의 평균 $m_b$ 및 표준 편차 $\sigma_b$를 취한 다음 각 개별 이미지의 파란색 값을 정규화한다.
...
여기서 $\epsilon$은 0으로 나누기를 피하기 위해 임의로 사용하는 숫자이다. 초록색 및 빨간색 채널에 대해서도 동일하게 사용하는게 좋다. 이것은 다른 조명에서 촬영 한 이미지에서 의미있는 신호를 얻는 데 필수적이다. 예를 들어, 자연광 사진에는 빨간색이 많고 수중촬영한 사진에는 거의 없다.
...
훈련이 진행됨에 따라 학습 속도를 감소시켜야한다. 실제 학습에 있어서, 대부분의 Advanced Model은 Adam과 같은 Optimizer를 사용하게 되는데 이것은 고정된 학습 속도를 쓰는 SGD이 아니라 적응형 학습속도를 사용하게 된다.

<!-- ### Use L1 and/or L2 regularization for weight decay -->
### Weight Deacy를 위한 L1, L2 정규화 사용
...
우리가 이것을 왜 weight decay라고 불르는지 이해하기 위해서는, 업데이트 중에 $w_i$에 1보다 작은 상수를 곱한 것을 확인 할 수 있게 위 식을 아래외 같이 적으면 알 수 있다.
...
L1 정규화 (Lasso) 역시 유사하다. $\Vert w \Vert^2$ 대신에 $\sum_i \vert w_i\vert$ 을 사용하는 차이가 있다.
<!-- 
...
드롭 아웃은 또 다른 형태의 정규화이다. 신경망의 또 다른 계층으로 생각할 수도 있다. 입력노드를 받아서 입력노드의 $n/2$ 를 임의로 0으로 설정하고 결과로 줄어든 출력 노드로 반환합니다(드롭아웃 확률이 0.5인 경우). 이로 인해 시스템은 적은 수의 입력 노드에 지나치게 의존하지 않고 모든 입력 장치에서 정보를 가져 와서 레이어의 모든 노드에 정보를 분배한다. 이 방법은 처음에 <a href="https://arxiv.org/abs/1207.0580"> Hinton et al (2012) </a>에서 제안 되었다.

더 많은 학습 트릭을 확인하고 싶으면 다음의 문서를 참고하길 바랍니다.  <a href="http://yann.lecun.com/exdb/publis/pdf/lecun-98b.pdf">LeCun et al 1998</a>.
...
마지막으로 역전파는 레이어가 쌓여 있는 Stacked 모델에 대해서만 작동하는 것은 아니다. 모듈에 부분적인 순서가있는 한 DAG (directed acyclic graph)에 대해 작동 할 수 있다/