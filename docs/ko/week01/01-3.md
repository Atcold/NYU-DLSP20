...
주피터 노트북은 [여기서](https://github.com/Atcold/pytorch-Deep-Learning/blob/master/02-space_stretching.ipynb) 찾을 수 있다. 노트북을 실행하려면 [`README.md`](https://github.com/Atcold/pytorch-Deep-Learning/blob/master/README.md)에 지정된 대로 `pDL` 환경이 설치되어 있어야 한다.
...
노트북 셀의 기능에 대한 설명서를 보려면 'Shift + Tab' 단축키를 사용하십시오.`
...
|                 (a) $s=1$ 일 때 비선형변환                 |                 (b) $s=5$ 일 때 빈선형변환                  |
