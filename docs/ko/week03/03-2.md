...
stride 1을 가지는 2개의 합성솝 신경망 계층과 stride 2를 가지는 2개의 풀링 계층으로 이루어진 CNN을 만들어 전체 stride가 4가 되도록 할 수 있다. 그러므로 새로운 출력값을 얻고싶다면, 입력 틀을 4만큼씩 움직여야한다. 더 정확히게 보자면, 아래 사진을 볼 수 있다. 처음에 입력값의 크기는 10이다. 여기에 크기 3의 합성곱을 수행해 8개의 단위를 얻는다. 그리고선 크기 2의 풀링을 실행해 4개의 단위를 얻는다. 이렇게 합성곱과 풀링을 반복하여 최종적으로 단 하나의 출력값을 얻을때 까지 계속한다. 
...
4개의 입력 계층이 있다고 가정해보자. 첫 합성곱 계층 이후에 4개의 단위를 더 가지고, 첫 풀링 계층 이후에 2개의 단위를 더 두번째 합성곱 계층에 2개더, 출력 계층에 1개 더 단위를 가질 수 있다. 새로운 출력을 만들기 위한 틀의 크기는 4 ((2 stride $\times$2))가 된다. 여기에 더해 이것은 우리가 입력값의 크기를 키우면, 모든 계층의 사이즈를 키우게 되고, CNN이 다양한 길이의 입력값을 감당할 수 있다는 것을 증명할 수 있게 된다. 
