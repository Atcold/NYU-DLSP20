...
얀 르쿤 박사님은 88년, 89년에 걸쳐 토론토 대학에서 첫 CNN 개발을 이뤄냈습니다. 이 연구는 쿠니히코 후쿠시마의 시각 피질 모델링에 영감을 얻어 단순/복잡 세포 위계를 지도 학습<sup>supervised training</sup>과 역전파<sup>backpropagation</sup>와 합침으로써 가능했던 결과였습니다. 해당 연구는 320개의 mouser-written 숫자들을 사용했습니다. 아래에 나열된 신경망 구조들의 퍼포먼스가 비교되었습니다:
...
1. 단일 완전 연결 계층
2. 다중 완전 연결 계층
3. 공유 가중치<sup>shared weights</sup>가 없는 지역적으로 연결된 계층들<sup>Locally Connected Layers</sup>
4. 로컬 연결과 공유 가중치가 있는 제한 신경망<sup>Constrained network</sup>
5. 로컬 연결과 공유 가중치가 있는 제한 신경망 2 (특징지도<sup>feature map</sup>가 더 많음)
...
가장 성공적인 결과인 신경망(공유 가중치가 있는 제한 신경망)이 가장 강한 일반화<sup>generalizability</sup> 가능성을 보였고,이 신경망이 현대적인 CNN들의 가장 기초가 됩니다. 반대로 단일 완전 연결 계층은 과적합<sup>overfit</sup>하는 경향을 보였습니다.
...
Bell Labs로 옮겨간 이후 르쿤 박사의 연구는 미국 우편국에서 얻은 수기 우편번호를 사용하는 것에서 더 큰 CNN으로 진행했습니다.
...
* 12 5$\times$5 의 stride 2 (2픽셀씩 옮겨감)를 가지는 커널 : 뒤에 오는 계층은 더 낮은 해상도를 가집니다.
* 개별적 풀링은 **하지 않음**
...
그 다음해, 몇개의 변경사항이 있었습니다: 개별적 풀링을 사용하게 되었습니다. 개별적 풀링은 입력 값들의 평균을 구하고, 편향치를 더하고, 이를 비선형함수에(하이퍼볼릭 탄젠트 함수<sup>hyperbolic tangent function</sup>)에 넣음으로써 이루어집니다. 2$\times$2 풀링은 stride 2에 의해 해상도를 반으로 줄임으로써 구현되었습니다.
...
단일 합성곱 계층의 예시입니다.
1. *32$\times$32* 의 입력값을 준비합니다.
2. 합성곱 계층이 stride 1과 5$\times$5의 커널을 이미지에 적용하여, *28$\times$28* 크기의 특징지도<sup>feature map</sup>를 만들어냅니다.
3. 비선형 함수에 특징지도를 넣습니다: 크기 *28$\times$28*
4. 2$\times$2 틀에 stride 2를 가지는 풀링 계층에 통과시킵니다: 크기 *14$\times$14*
5. 4개의 커널에 대해 1-4단계를 반복합니다.
...
보통 간단한 합성곱/풀로 이루어진 첫번째 계층은 윤곽과 같은 간단한 특징을 감지합니다. 그 다음 계층의 목표는 이전 계층에서 넘어온 특징들의 조합을 감지하는 것입니다. 이런 감지를 해내기위해 여러개의 커널을 이용해, 바로 이전 계층이 만들어낸 특징지도를 대상으로 위에서 서술한 2 ~ 4단계를 반복하여 새로운 특징지도로 재조합됩니다.
...
1. 새로운 5$\times$5 커널이 이전 계층의 모든 특징 지도들을 처리하면서 결과를 모두 더합니다. (참고: 1989년도에 진행된 르쿤 박사의 실험에서는 계산 목적으로 모든 계층을 완전히 연결하지 않았습니다. 현대의 환경은 보통 완전히 연결되도록 강제합니다.) : 크기 *10$\times$10*
2. 합성곱의 결과값을 비선형 함수에 집어 넣습니다: 크기 *10$\times$10*
3. 1,2 단계를 16개의 커널에 대해 반복합니다.
4. 결과값을 stride 2값을 가지는 2$\times$2 틀로 평균값을 내는 풀링 계층에 전달합니다: 각 특징 지도의 크기 *5$\times$5*
...
결과값을 만들어내기 위해서 합성곱 신경망의 가장 마지막 계층이 적용됩니다. 이 계층은 완전히 연결된 모양으로 보이겠지만, 사실은 합성곱입니다.
...
1. 최후의 합성곱 계층이 5$\times$5 크기의 커널을 이용해 결과값들을 더하며 모든 특징지도를 훑습니다: 크기 *1$\times$1*
2. 비선형 함수에 넣습니다: 크기 *1$\times$1*
3. 하나의 분류에 해당하는 단일 결과를 만들어냅니다.
4. 10개의 분류에 대해서 이전의 모든 단계를 반복합니다(동시에).
...
이 Andrej Karpathy의 웹사이트에 있는 [애니메이션](http://cs231n.github.io/convolutional-networks/)에서 어떻게 합성곱이 다음 계층의 모양을 바꾸는지 보실수 있습니다. 완전한 논문은 [여기](https://papers.nips.cc/paper/293-handwritten-digit-recognition-with-a-back-propagation-network.pdf)에서 보실 수 있습니다..
...
슬라이드에 있는 애니메이션에 보여지는 것처럼 입력 이미지를 변형시키는 것은 특징지도에도 같은 변형을 일으키게 됩니다. 그러나, 특징지도의 변화는 합성곱/풀의 작동에 의해 조절되게 됩니다. 예를 들어 stride 2값을 가지는 2$\times$2의 풀링은 입력 계층의 1-픽셀 전이를 특징 지도의 0.5픽셀 전이로 줄여 계산합니다. 분해능<sup>spation resolution</sup>은 특징 종류에 따라 증가된 숫자에 교환됩니다.  
...
일반 CNN 구조는 여러가지 기본적인 기본 계층 구조로 나눠볼수 있습니다.
...
LeNet5에는 다음과 같은 계층들이 포함됩니다. (1이 가장 윗 계층이 됩니다.):
...
입력은 32$\times$32 흑백1도의 이미지 입니다. (1 인풋 채널)

<!-- LeNet5 can be implemented in PyTorch with the following code: -->
LeNet5 는 아래 코드를 통해 PyTorch에 구현할 수 있습니다.
...
`fc1`와 `fc2`는 완전 연결 계층이지만, 커널이 인풋 전체 크기만한 합성곱으로 빗대어 생각해볼 수도 있습니다.
완전 연결 계층은 효율성을 위해 사용됩니다.

<!-- The same code can be expressed using `nn.Sequential`, but it is outdated. -->
위와 같은 코드는 `nn.Sequential`를 이용해서도 구현할 수 있지만, 오래된 방법입니다.
...
완전 연결된 합성곱 신경망에서 입력 사이즈를 정의할 필요는 없습니다. 그렇지만, 입력 사이즈를 바꾸면 출력 사이즈도 변하게 됩니다.
...
필기체 인식 시스템을 상상해 봅시다. 입력 이미지를 작은 단위로 나눌 필요는 없이, 전체 이미지에 CNN을 적용하면 됩니다: 커널이 이미지 전체의 모든 부분을 감싸게 되고, 패턴이 어디에 있든 같은 출력을 기록하게 될겁니다. CNN을 이미지 전체에 적용하는 방법은 여러 부분에 각각 적용하는 것 보다 훨씬 경제적입니다. 다행히도 이미지를 인식하는 것과 거의 비슷한 일인 이미지를 분할하는 일은 굳이 하지 않아도 됩니다.   
...
LeNet5는 가운데 위치한 각각의 숫자를 인식할 수 있도록 32$\times$32 크기의 MNIST 이미지를 학습했습니다. 숫자를 이리저리 옮기고, 사이즈를 변화시키거나, 귀퉁이에 숫자를 끼워넣음으로써 데이터를 확장합니다. 아무것에도 해당하지않음을 표시하는 11번째 카테고리를 포함하여 학습하기도 했습니다. 이 카테고리로 라벨링이 된 이미지는 검은 이미지를 생성하거나 귀퉁이에만 숫자를 배치함으로써 생성되었습니다. 
...
위의 이미지는 32$\times$32에 LeNet5으로 학습한 신경망이 32$\times$64 입력 이미지에 있는 다양한 숫자를 인식할 수 있음을 보여줍니다.
...
시각 신경학자와 컴퓨터 비전 연구자들은 물체를 물체로 정의하는 법에 대한 문제를 겪고 있습니다. 물체는 특징의 총체지만, 어떻게 특징들을 하나로 묶어 하나의 물체가 되도록 만들 수 있을까요?
...
이 특징 바인딩 문제는 아주 간단한 CNN을 통해 해결할 수 있습니다: 단 두개의 풀링과 합성곱 계층과 
...
위의 애니메이션은 CNN이 단 하나의 선을 움직이며 다양한 숫자를 인식하는 모습을 보여주며, 어떻게 특징 바인딩 문제를 해결할 수 있는지 보여줍니다. 위계적으로, 조합적으로 특징을 인식하는 방법을 보여줍니다.
...
4개의 입력 계층이 있다고 가정해봅시다. 첫 합성곱 계층 이후에 4개의 단위를 더 가지고, 첫 풀링 계층 이후에 2개의 단위를 더 두번째 합성곱 계층에 2개더, 출력 계층에 1개 더 단위를 가질 수 있습니다. 새로운 출력을 만들기 위한 틀의 크기는 4 ((2 stride $\times$2))가 됩니다. 여기에 더해 이것은 우리가 입력값의 크기를 키우면, 모든 계층의 사이즈를 키우게 되고, CNN이 다양한 길이의 입력값을 감당할 수 있다는 것을 증명할 수 있게 됩니다. 
...
CNN은 다차원 행렬로 정보를 받을때 유용합니다. 이런 정보들은 3가지 주요 특성을 가집니다.
...
1. **지역성**: 첫번째 특성은 각 값들 사이에 강한 지역적 연관성이 있다는 것입니ㅏㄷ. 만약 우리가 원본 이미지에서 서로 인접한 두개의 픽셀을 선택한다면, 이 두개의 픽셀은 같은 컬러값을 가질 확률이 매우 높습니다. 이 두개의 픽셀의 거리가 점점 먼것일 수록, 두개 사이의 연관성은 줄어들게 됩니다. 이런 지역적 연관성은 우리가 지역적 특징을 찾는데 도움을 줍니다. 이것이 바로 CNN이 하는 일이죠. 만약 우리가 CNN에 순열 이미지를 사용한다면, 완전 연결 계층은 별로 영향을 받지 않을 것이고, 입력 이미지를 인식하는 일을 잘 하지 못할겁니다. 지역적 연관성이 있기에 지역적 연결이 있을 수 있습니다.
...
2. **고정성** : 두번째 특성은 특징은 필수적이고 이미지 어디에나 나타남으로써 공유 가중치와 풀링을 정당화합니다. 더 나아가 정적인 정보들은 균일하게 배포되기 때문에, 우리는 모든 특징 감지를 입력 이미지 모든 곳에서 수행해야합니다.
...
3.  **구성성**: 세번째 특성은 특징들이 각각 위계를 이루는 형태고 모여 이미지를 구성하고 있다는 점입니다. 이런 특성이 여러개의 신경을 가진 다층 계층을 사용하는 이유를 설명해주며, 이것은 허벨과 와이젤의 단순/복잡 세포에 대한 연구와도 대응되는 부분입니다.

<!-- Furthermore, people make good use of CNNs on videos, images, texts, and speech recognition. -->
여기에 더해 사람들은 비디오, 이미지, 텍스트, 발화 이해에 CNN을 잘 이용하고 있습니다.