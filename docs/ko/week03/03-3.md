...
언어 또한 위에서 보여주었던 방식으로 접근이 가능하다. 각 단어는 특정 용어에 존재한다면 1, 아닌 경우에는 전부 0인 원-핫벡터<sup>one-hot vector</sup>로 표현가능하다. 즉, 각 단어는 특정 용어 크기만큼의 벡터인 것.
...
2. 집약성<sup>Locality</sup>
	- 가까운 포인트들끼리는 거리가 비교적 더 먼 곳에 있는 포인트보다 더 연관성을 띰. 1차원 신호의 경우, 집약성은 $t_i$에 위치한 한 임의에 고점 $x_{t_i}$와 그 고점의 근방은 비슷한 값을 가지며, $t_i$와 먼곳에 떨어진 $t_j$에 위치한 $x_{t_j}$에는 영향력이 미미함을 뜻한다. 좀 더 자세하게는, 한 신호와 다른 반전된 신호가 완벽히 겹쳐지게 된다면 그 두 신호의 컨벌루션(합성곱)은 최곳값을 갖는다. 두 개의 1차원 신호들의 합성곱(상호상관<sup>cross-correlation</sup>)은 두 신호의 스칼라곱이며, 두 벡터가 얼마나 비슷한지 혹은 가까운지를 측정하는 하나의 측정법이다. 그 말인 즉 슨, 정보는 신호의 특정한 구간과 부분에 들어있는 것이며, 사진의 경우, 한 이미지의 두 점 간의 상관도는 두 점이 더욱이 멀리 떨어질수록 낮아지는 것이다. 예로, 한 이미지의 $x_{0,0}$ 픽셀이 파란색이면, $x_{0,0}$ 근방의 픽셀들의 색이 파란색일 확률은 매우 높으며, $x_{0,0}$픽셀과 멀어지면 멀어질수록 $x_{0,0}$의 색값과 독립성을 띤다.
...
가진 데이터가 정상성, 집약성, 합성성을 띤다면 희소성<sup>sparsity</sup>, 가충치 공유<sup>weight sharing</sup>, 층 쌓기<sup>stacking of layers</sup>를 이용한 신경망을 통해 위의 성질들을 알아낼 수 있다. 
...
## [불변<sup>invariance</sup>과 등변<sup>equivariance</sup>을 위한 자연 신호의 성질 알아내기](https://www.youtube.com/watch?v=kwPWpVverkw&t=1074s)
...
Fig.1 은 5중 계층의 완전 연결망<sup>fully connected network</sup>을 보여준다. 각 화살표는 입력값들과 곱하여진 가중치를 나타내며, 보이는 것과 같이 위 신경망은 계산 비용이 많이 들어간다.
...
가진 데이터가 집약성을 띤다면, 각 뉴런은 전 레이어들의 소수의 로컬 뉴런에만 연결되어야한다. 그러므로, 일부 연결들은 Fig.2(b)에서 보이는 것처럼 떨어져 나갈 수 있다. (Fig.2(a)는 전연결 신경망 FC network를 설명함). 이러한 집약성 특성의 장점을 이용해 거리가 먼 뉴런들과의 연결들을 끊을수 있고, Fig.2(b)의 히든 레이어 뉴런들은 모든 입력값을 생성<sup>span</sup>하지는 않지만, 전반적인 구조는 모든 입력 뉴런들을 설명할 수 있다. 
...
Fig.4는 사이즈가 2(커널 수), 7(전 레이어의 두께), 3(고유 연결/가충치의 수)인 1차원 데이터 커널의 한 예이다.
...
커널 사이즈 선택은 실증적으로 유추해낼 수 있다. 예로, 3 * 3 합성곱은 공간데이터의 최소크기로 보이지만, 크기가 1인 합성곱은 더 큰 입력 사진에 적용가능한 마지막 레이어를 얻기위해 사용되며, 짝수의 커널 사이즈는 데이터의 질을 떨어뜨릴 수 있으므로, 커널 사이즈는 항상 3과 5같은 홀수로만 이루어진다.
...
패딩은 전반적으로 최종 결과물을 떨어뜨리지만, 프로그래밍적으로는 편리한 방법이다. 보통은 제로패딩<sup>zero-padding</sup>을 이용하는데 계산법은 아래와 같다 `사이즈 =  (커널 사이즈 - 1)/2`
...
배치 정규화와 잔차 우회연결은 연결망을 학습시키는데 정말 많은 도움이 된다. 레이어가 너무 많이 쌓여있는 경우에는 신호가 부분적으로 갈 길을 잃어버릴수도 있기 때문에, 잔차 우회로 인한 추가적인 연결들이 밑에서 위까지의 길을 확보해주고. 위에서부터 아래까지 경사도의 길 또한 확보해준다. 
...
Fig.5를 보면, (각 픽셀의 색과 같은 특징정보를 별개로) 입력 사진이 2개의 차원에 걸쳐 대부분의 공간정보<sup>spatial information</sup>를 가지고 있지만, 출력 레이어는 굵다. 중간 과정을 거치며, 공간정보와 특징정보<sup>characteristic information</sup>간의 트레이드 오프가 존재하며 표현은 점점 밀집되어 가며, 계층을 올라갈 수록, 희소정보를 잃음으로서 조금 더 높은 밀집표현<sup>dense representation</sup>을 갖게된다.
...
$L_p$-노름<sub>norm</sub>은 여러 다른 지역에 적용이 되며 각각의 지역에 오직 한 값을 주고 (책의 예시는 4개의 픽셀에 하나의 값), 전체 데이터에 지역과 지역별로 임의로 정한 깊이를 바탕으로 반복적용된다. 
예시로 만일 $c$개의 채널을 가진 $m * n$ 데이터를 이용한다면, 결과로 $c$개의 채널수를 유지한 $\frac{m}{2} * \frac{n}{2}$ 데이터를 얻게된다 (Fig.7). 풀링은 매개변수화 하지는 않지만 맥스풀링<sup>max pooling</sup>, 평균풀링<sup>average pooling</sup> 등 여러가지의 풀링 타입을 정할 수 있고, 풀링을 사용하는 주된 목적은 데이터의 양을 줄임으로써 합리적인 계산시간을 갖게하는 것에 의의를 둔다.  
...
위 노트북을 실행하기에 앞서 `pDL` 환경이 정확한 [`README.md`](https://github.com/Atcold/pytorch-Deep-Learning/blob/master/README.md). 에 설치되있는지 확인요망.
...
위 노트북은 MNIST 데이터셋의 분류작업을 위해 다층 퍼셉트론(전열결 신경망)과 합성곱 신경망(CNN)을 학습시켰으며 두 신경망 모두 같은 수의 매개변수를 가지고 있음을 알아두시기 바람(Fig.8).
...
학습시키기에 앞서, 데이터 정규화를 통해 신경망의 초기치 설정이 데이터 분포에 맞게 해주었다 (매우 중요!).
또한, 아래의 5가지 작업/단계가 학습 과정에 포함되었음을 유념 바람:
...
첫째로, 두 신경망을 정규화된 MNIST 데이터에 학습시켰고, 그 결과, 다중 퍼셉트론 신경망은 87%의 정확도를, CNN은 95%의 정확도를 가졌으며, 주어진 매겨변수의 수가 같음에도 불구하고, CNN은 다중 퍼셉트론보다 더 많은 필터를 학습할 수 있었다. 다중 퍼셉트론 신경망에서는 가까운 물체들과 먼 곳에 위치한 물체들의 의존성을 찾는 필터를 학습시키는 아주 쓸데없는 작업을 진행하였음에 반해, 합성곱 신경망 같은 경우에는, 모든 매개변수가 근접한 픽셀 간의 관계를 찾는 작업에 집중했음을 찾아볼 수 있다.
...
다음으로, MNIST 데이터셋 모든 이미지의 모든 픽셀에 랜덤순열<sup>random permutation</sup>을 생성하였고, 이들은 Fig.8과 Fig.9과 같이 변형되며, 다중 퍼셉트론 신경망과 CNN을 다시 이 수정된 데이터셋에 학습시켰다.
...
<center><img src="{{site.baseurl}}/images/week03/03-3/Figure 9 Instances from Permuted MNIST Dataset.png" width="350px" /><br>
<b>Figure 9:</b> 순열 후 MNIST 데이터셋의 인스턴스 </center>
...
다중 퍼셉트론 신경망의 성적은 85%로 변화를 보이지 않았으나, CNN의 정확도는 83%로 떨어졌는데, 이와 같은 현상이 벌어진 이유는 랜덤수열 이후, 사진들이 더는 CNN을 통해 얻을 수 있는 집약성, 정상성, 합성성의 특징을 보이지 않아서이다. 
