...
빔 서치는 텍스트를 생성하고, 언어 모델을 디코딩하는 또 다른 테크닉이다. 모든 스텝마다, 알고리즘은 $k$개의 가장 가능성있는 (최선의) 번역 부분들(가설들<sup>hypotheses</sup>)을 꾸준히 추척<sup>track</sup>한다. 각 가설의 스코어는 그의 로그 확률<sup>log probability</sup>과 동일하다. 
...
빔 트리는 문장 토큰의 끝에 도달할때까지 이어진다. 문장의 마지막 토큰을 내놓으면서, 가설도 종료된다.

어째서 매우 크기가 큰 빔 사이즈들은 (NMT에서) 종종 빈 번역 결과를 내놓을까?

트레이닝을 할 때, 빔이 비싸기 때문에 알고리즘은 자주 사용하지 않는다. 대신에 자동 회귀 분해<sup>auto-regressive factorization</sup> (이전의 올바른 결과물들을 받아, $n+1$ 첫번째 단어들을 예측한다.)을 사용한다. 모델은 트레이닝 중에는 자신의 실수들을 노출시키지 않는다, 그래서 빔에서 "무의미한 말"을 내놓을 가능성이 있는 것이다. 
...
하지만, 모델의 분포로부터 샘플링을 하는 것은 이것이 가진 고유한 문제를 드러낸다. 한 번 "나쁜" 선택이 샘플링되었을 때, 모델은 트레이닝 동안 마주친 적이 없는 상태에, 연속적으로 "나쁜" 분석의 우도를 증가시키면서 있다. 그래서 알고리즘은 끔찍한 피드백 루프에 갇힐 수 있다.
...
샘플링 기술 자체는 분포를 앞 뒤로 잘라서 $k$개의 최상의 것을 뽑고, 다시 정규화를 하고 그 분포로부터 샘플링을 하는 것이다. 
...
#### 질문 : 왜 톱-K 샘플링은 이렇게 잘 되는 것일까?

이 테크닉이 잘 되는 이유는 본질적으로, 꼬리를 잘라낸 분포의 머리 부분만 사용하느라 뭔가 안 좋은 것이 샘플링 되려고 할 때, 즉, 좋은 언어의 매니폴드<sup>manifold</sup>에서 떨어지는 일이 발생하게 하지 않으려 노력하기 때문이다.
...
언어 모델을 평가하는 일은 들고 있는 데이터의 로그 우도<sup>log likelihood</sup>만 필요로 한다. 하지만, 텍스트를 평가하는 일은 어렵다. 보편적인 단어 오버랩 메트릭들<sup>word overlap metrics</sup>은 레퍼런스(BLEU, ROUGE etc.)와 함께 쓰이지만, 각자가 가진 문제들도 있다.
...
조건부 언어 모델들은 영어로부터 무작위한 샘플을 생성해내는데에는 별로 쓸모가 없다. 하지만 주어진 입력값에 대한 텍스트를 생성해내는데에는 유용하다. 
...
결과값에 있는 모든 토큰은, 거기에 있는 모든 이전 토큰들과 또, 입력값의 모든 단어들과 직접적으로 연결되어 있다. 이러한 연결이 모델을 매우 강력하게 또 표현력있게 만든다. 트랜스포머들은 이전의 순환 모델과 합성곱 모델들을 너머, 번역 점수의 향상을 이루어냈다. 
...
이 모델들을 트레이닝할 때, 우리는 보통 많은 양의 라벨링된 텍스트에 기대게 된다. 유럽 의회의 절차는 좋은 데이터 소스 공급자이다 - 텍스트는 서로 다른 언어들로 수동으로 번역되어, 모델의 입력값과 출력값들로 사용할 수 있다.
...
- 모든 언어들이 유럽 의회에 나타나는 것이 아니다, 이는 우리가 관심있어하는 모든 언어들을 위한 번역 페어<sup>translation pair</sup>를 얻지 못할 수도 있다는 것이다. 굳이 데이터를 얻으려고 (노력)할 필요없이, 어떻게 트레이닝을 위한, 언어 텍스트를 찾을 수 있을까?

- 트랜스포머같은 모델들은 더 많은 데이터가 있을 때 더 좋은 성능을 내는데, 우리는 어떻게 단일 언어 텍스트(댜른말로, 입력값이나 출력값이 없는 페어들)를 효율적으로 사용할 수 있을까? 

우리가 독일어에서 영어로 번역하는 모델을 트레이닝시키고 싶다고 가정해보자. 역-번역의 아이디어는 영어에서 독일어로 가는 역 모델을 첫번째로 트레이닝하는 점에 있다. 

- 어떤 제한된 이중 언어-텍스트를 사용함으로써 우리는 2개의 다른 언어로 된 같은 문장을 얻을 수 있다.
- 한 번 영어-독일어 모델을 가지면, 많은 단일 언어 단어들을 영어에서 독일어로 번역할 수 있다. 

마지막으로, 이전 스텝에서 '역-번역'한 독일어를 사용해서 독일어에서 영어로 가는 모델을 트레이닝한다. 알아두어야 할 부분들은:

- 역 모델이 얼마냐 좋으냐와는 상관없다 - 엉망인 독일어 번역을 얻겠지만, 결과적으로는 깨끗한 영어를 번역할 수 있을 것이다. 
- 영어/독일어 페어(이미 번역되어 있는)의 데이터 넘어, 영어를 잘 이해하는 것을 배울 필요가 있다 - 많은 양의 영어 단일 언어 데이터를 사용해라
...
- 많은 양의 병행 데이터를 쓸 수 없을 때 큰 도움이 된다. 
...
- 하나의 언어로부터 다른 언어로의 번역을 배우려고 노력하는게 아니라, 대신 다양한 언어 번역을 배우도록 신경망을 구축한다. 
- 모델은 일반적이고 언어와는 독립되어 있는 정보를 배우게 된다. 
...
<!-- Intuition - if words appear close together in the text, they are likely to be related, so we hope that by just looking at unlabelled English text, we can learn what they mean.
 -->
직관적인 이해 - 텍스트에서 단어들이 근접해서 나타나면, 그들은 연결되어 있을 가능성이 높다, 그러므로 우리는 단순히 라벨링되지 않은 영어 텍스트를 읽는 것만으로 그들의 뜻을 배울 수 있기를 바란다. 
...
Interpretation of words depends strongly on context. So in the instance of ambiguous words - words that may have multiple meanings - the model will struggle since the embeddings vectors won't capture the context needed to correctly understand the word.
 -->
#### 질문: 이 모델이 난관을 겪을 상황에 대한 예시가 있을까?

단어를 통역하는 일은 맥락에 크게 좌우된다. 그러므로 애매한 단어들에 대한 예시에서 - 단어들은 여러가지 뜻을 가질 수 있으므로 - 임베딩 벡터들은 단어를 올바르게 이해하기 위해 요구되는 맥락을 알 수 없으므로, 모델은 고생을 하게 될 것이다. 
...
맥락을 더하기 위해서, 우리는 조건부 언어 모델을 트레이닝한다. 그리고 이 언어 모델이 주어진 상태에서, 각 타임스텝마다 단어를 예측하고, 모델의 각 출력값을 다른 어떤 특징으로 대체한다. 
...
- 사전 트레이닝 - 다음 단어 예측
- 파인-튜닝 - 세부 과제로 바꾸는 것이다. 예시들:
  - 명사인지 형용사인지 예측
  - 아마존 리뷰들로 구성된 어떤 텍스트를 주고, 리뷰에 대한 감성 점수를 예측한다
 
<!-- This approach is good because we can reuse the model. We pretrain one large model and can fine tune to other tasks. -->
이 접근법은 좋은데, 모델을 재사용할 수 있어서이다. 하나의 거대한 모델을 사전에 트레이닝하면, 다른 과제들을 위한 파인 튜닝을 할 수 있다.
...
- 하나는 왼쪽에서 오른쪽으로 향한다.
- 하나는 오른쪽에서 왼쪽으로 향한다.
- 단어의 표현<sup>word representation</sup>을 얻기 위해, 두 모델의 결과물을 연결한다. 이제 우리는 우측으로부터의 맥락과 좌측으로부터의 맥락 모두에 의존한다. 

이것은 여전히 "얕은" 조합이며, 왼쪽과 오른쪽의 맥락 사이에서 좀 더 복잡한 상호 작용이 있었으면 좋겠다.
...
버트<sup>BERT</sup>는 이전에 한 공백-채우기 측면에서, 워드투벡터와 비슷하다. 그러나 워드투벡터에서는 선형 투영을 했지만, 버트에는 더 많은 맥락을 볼 수 있는 거대한 트랜스포머가 있다. 트레이닝을 위해, 우리는 토큰 중 15%는 마스크를 할 것이고, 이 빈 부분들을 예측하려고 할 것이다. 
...
최고의 버트 퍼포먼스 결과에서도 더 큰 향상을 보임 - 질문-답변 과제의 퍼포먼스에 있어서는 현재 인간 수준을 넘어선 상태이다.
...
- 알버트<sup>ALBERT</sup>:
  ...
  경량화된 버트: 우리는 레이어들을 교차하는 가중치들을 묶는 것으로, 버트를 수정하고 더 가볍게 만들었다. 이는 모델의 파라미터들과 관련된 계산들을 줄인다. 재미있게도, 알버트의 저자들은 정확도에 대해서 크게 타협할 필요가 없었다.
  
- XLM:
    
  다중 언어 버트: 영어 텍스트같은 것을 넣는 대신, 여러 언어로부터 온 텍스트를 넣었다. 예측한대로, 교차적인 언어 연결을 더 낫게 학습했다.

키는 위에서 언급된 서로 다른 모델로부터 가져온다

- 많은, 서로 다른 사전학습 목표들이 잘 된다!

- 단어간의 깊고, 양방향의 상호 작용을 하게 하는 건 모델에게 굉장히 중요하다

- 스케일을 키운 사전 학습으로부터 큰 이득을 본다. 아직 확실한 제약사항은 없다.

위에서 논의된 대부분의 모델들은 텍스트 분류 문제를 풀도록 설계되었다. 하지만, 텍스트 생성 문제를 풀기 위해서는, `시퀀스-투-시퀀스` 모델처럼 결과값을 연속적으로 생성해야하는지라, 사전 학습을 위한 약간의 다른 접근법이 필요하다.
...
이 `시퀀스-투-시퀀스` 사전학습 접근법은 우리의 망가진 스킴 디자인 측면에서 유연함을 제공한다. 문장을 섞거나, 구절을 없애거나, 새로운 구절을 소개하는 등의 일들을 할 수 있다.

바트는 GLUE와 SQUAD 과제에서 로베르타<sup>RoBERTa</sup>를 상대할 수 있었다. 그러나, 그건 요약, 대화와 추상화된 QA 데이터셋에서 새로운 SOTA였다. 이 결과들은 텍스트 생성에서 (바트가) 버트/로베르타보다 더 나아지도록 하는, 우리의 동기부여를 강화한다.
...
"트로피는 너무 크기 때문에, 여행가방에는 맞지 않다.": 기계들이 이 문장에서 '그것'이 뭘 가리키는지를 알아내기란 어려운 일이다. 인간들은 이 과제를 잘 해낸다. 이런 어려운 예시들로 구성된 데이터셋에서 인간은 95%의 성능을 달성했다. 컴퓨터 프로그램은 트랜스포머가 혁신을 가져오기 전까지, 고작 60% 언저리의 성능만을 달성했을 뿐이다. 최근의 트랜스포머 모델들은 저 데이터셋에 대해 90% 이상을 달성할 수 있다. 이는 이 모델들이 데이터를 단순히 암기/활용만 하는 것이 아니라, 데이터 내부의 통계학적인 패턴을 따라 개념과 목표들을 학습하고 있음을 시사한다. 
...
흥미롭게도, 강의자 (Mike Lewis, Research Scientist, FAIR)는 '기초 언어'라고 불리우는 개념을 연구하고 있다. 이 연구 분야의 목적하는 바는 칫-챗<sup>chit-chat</sup>이나 협상 가능한, 대화가 가능한 에이전트를 만드는 것이다. 칫채팅<sup>chit-chatting</sup>과 협상을 한다는 건, 텍스트 분류나 텍스트 요약과 비교해서 (상대적으로) 불확실한 목표들과 함께해야 하는 추상 과제들이다.
...
'세계 지식'은 추상적 개념이다. 우리가 관심있는 개념들에 대해 간단한 질문들을 모델에 하는 식으로, 매우 기본적인 수준에서만 그들의 세계 지식을 테스트할 수 있다. 버트나 로베르타, T5같은 모델들은 수십억개의 파라미터를 갖고 있다. 이런 모델들을 위키피디아같은 비격식적인 텍스트들의 거대한 언어 자료로 트레이닝하는 걸 생각해보면, 파라미터들을 사실을 암기하는데에 사용하고는 우리가 하는 질문들에 대해 답변 할 수 있을 것이다. 추가로, 어떤 과제에 대해 모델을 파인튜닝하기 전과 후에, 동일한 지식 테스트를 (모델들에) 수행하는 것도 생각해볼 수 있다. 이는 얼마나 많은 정보들을 모델이 '망각'해야하는지에 대한 감을 우리에게 전해준다. 