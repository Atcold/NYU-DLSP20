...
Bu bölümde, verilerin esnekliğini, durağanlığını, bileşimini araştırmak istediğimiz için evrişimi tartışacağız.
...
[Önceki hafta] 'da tartışılan $ A $ matrisini kullanmak yerine ({{site.baseurl}} / tr / week04 / 04-1) matris genişliğini $ k $ çekirdek boyutuna değiştireceğiz. Bu nedenle, matrisin her satırı bir çekirdektir. Çekirdekleri istifleyerek ve kaydırarak kullanabiliriz (bkz. Şekil 1). Sonra $ m $ katmanları olabilir $ n-k + 1 $.
...
Çıktı, $ n-k + 1 $ boyutunda $ m $ (kalınlık) vektörleridir.
...
burada $ \ Omega = \ lbrace 1, 2, 3, \ cdots \ rbrace \ subset \ mathbb {N} ^ 1 $ (bu 1 $ $ boyutlu sinyal olduğu için / 1 $ $ boyutlu bir alana sahip olduğu için) ve bu durumda kanal $ c $ sayısı 1 $ $. $ C = 2 $ olduğunda bu bir stereofonik sinyal olur.

For the 1D convolution, we can just compute the scalar product, kernel by kernel (see Fig 4).

1D evrişim için sadece skaler ürünü, çekirdeği hesaplayabiliriz (bkz. Şekil 4).
