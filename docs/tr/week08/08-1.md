...
Maximum Likelihood yöntemi olasılıksal bir şekilde eğitim kümesindeki verilerin enerjilerini aşağı çekerken geri kalan her yerde enerjileri yukarı doğru iter. Bu yöntemö enerjilerin "mutlak değerleri"nden ziyade "aralarındaki farkı" dikkate alır. Bunun sebebi ise olasılık dağılımlarının toplamının/integralinin her zaman 1 edicek şekilde normalize edilmesidir ki bu durumda herhangi iki noktanın oranına bakmak mutlak değerlerini kıyaslamaktan daha yararlıdır.
...
Karşıtsal yöntemlerde eğitim kümesinden gözlemiş olduğumuz örneklerin ($x_i$, $y_i$) enerjilerini aşağı iterken, eğitim kümesinin manifoldu dışında kalan örneklerin enerjlerini yukarı itiyoruz.
...
Bir çift ($x$, $y$) düşünelim, böylece $x$ is an görüntü ve $y$'de $x$ 'in içeriği koruyan bir dönüşüm i
(döndürme, büyütme, kırpma, * vb. *) olsun.  Bu çifti **pozitif** çift olarak adlandıralım.
...
Konsept olarak karşıtsal gömulme yöntemleri, pozitif çifti evrişimsel bir ağın girdisi olarak kullanır ve  iki öznitelik vektörü $h$ and $h'$ öğrenir. Pozitif çift $x$ and $y$ aynı içeriğe sahip olduğu için, öznitelik vektörlerinin de olabildiğince benzer olmalarını istiyoruz. Bu amacı gerçekleştirmek için, benzerlik metriği (kosinüs benzerliği gibi) ve $h$ ile $h'$ arasındaki benzerliği maksimize eden bir kayıp fonksiyonu tanımlamamız gerekiyor. Bunu yaparak, eğitim verisini barındıran manifolddun içinde bulunan tüm görüntülerin enerjilerini düşürüyoruz.
...
Yukarıdaki şekilde de görüldüğü üzere, MoCo ve PIRL SOTA sonuclara ulaşmıştır (özellikle az sayıda parametreli ve düşük kapasiteli modellerde). PIRl, diğer denetimli sistemlerin doğruluğuna(~75%) yaklaşmaya başlamıştır. 
...
PIRL yönteminin amaç fonksiyonu aşağıdaki gibi olan NCE'yi(Gürültüye Karşıt Kestirici, Noise Contrastive Estimator) inceleyerek daha iyi anlayabiliriz.
...
Dr. LeCun, PIRL'ın çalışması için oldukça fazla sayıda negatif örneğe ihtiyaç uyulduğunu belirtmektedir. SGD'de mini-batchlerde negatif örneklerin çok sayıda parçasını sürekli olarak tutabilmek zor olabilir. Bü  durumu çözmek için PIRL, özel bir ön bellek yapısı kullanmaktadır.
...
Karşıtsal Iraksama'nın özel bir versiyonu ise Kalıcı Karşıtsal Iraksama yöntemidir. Önceki yönteme kıyasla sistem, konumunu hatırlayabilen bir grup  "parçacık" kullanır. Bu parçacıklar tıpkı CD'daki gibi enerji yüzeyinde hareket ettirilir. Sonunda, bu parçaçıklar enerji yüzeyimizdeki düşük enerjili yerleri bulucak ve yukarı itilmelerni sağlayacaktır. Ancak, boyut arttıkça sistem performansı kötü etkilenmektedir.
