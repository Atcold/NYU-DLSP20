...
Saklı değişkenlere sahip modeller kestirimlerin $\overline{y}$ gözlemlenen girdi $x$ ve ilaveten bir *saklı değişken* $z$ 'ye  koşullu dağılımını üretebilirler.
Enerji bazlı modeller de saklı değişken içerebilir:
...
Bu yöntem, küçük değerler alan z'nin uzay hacmini sınırlayacak ve bu sınırlamayla düşük enerjiye sahip y'nin uzayını da kontrol ddebilmiş olacağız. Lambda'nın değeri bu dengeyi kontrol eder. Reel syılar kümesi $R$ için örnek vermemiz gerekirse $L_1$ normu,  which can be viewed as an almost everywhere differentiable approximation of effective dimension. $z$'nin $L_2$ normunu kısıtlarken üzerine gürültü eklemek de bilgi kağasitesini sınırlar(VAE).
...
The $n$ boyutlu vector $z$ sıfırdan will tend to have a maximum number of non-zero components $m << n$. Böylece, her $Wz$ will be elements in the span of $m$ columns of $W$.
...
Her eniyileme (optimızation) adımından sonra, matris $W$ ve saklı değişken $z$ , $W$'nun sütünlarının  $L_2$ normları toplamı ile normalize edilir. Bu işlemi yaparak, $W$ ve $z$'nin sıfıra veya sonsuza ulaşmamasını garanti etmiş oluruz.
...
Yukarıdaki kuralı yinelemeli sinir ağı gibi yorumlamak mümkün çünkü saklı değişkken $z$'yi   
This update rule can be interpreted as a recurrent network, which suggests that we can instead learn the parameters $W_e$ that iteratively determine the latent variable $z$.Ağ, sabit sayıda $K$ adımında çalıştırılır ve  $W_e$'nin gradyanları ile standard backpropagation-through-time hesaplanır. Eğitilmiş ağ, FISTA algoritmasından daha kısa sürede iyi $z$ üretir. 
...
256 boyutlu bir saklı değişkene sahip seyrek kodlama sistemi, MNIST verisetinde kullanıldığında sistem eğitim kümesinin neredeyse tamamını oluşturabilecek kalem  the system learns a set of 256 strokes that can be linearly combined to nearly reproduce the entire training set. Seyrek düzenleştirici, yeni örnekleri oluşturuken olabildiğince az vuruş kullanmaya zorlar.
...
Değişimsel Otokodlayıcılar, seyreklik haricinde düzenlileştirilmiş saklı seğişken enerji bazlı modellerinkine benzer bir mimariye sahiptir. Aksine bilgi, koda gürültü ekleyerek sınırlandırılır.
...
Sistem, $z$'nin(gürültü) etkisinin olabildiğince azaltmak için kod vektörleri  ${\overline z}$'yi mümkün olduğunca büyütmeye çalışır. Bu da , Şekil 9(b)'de görüldüğü gibi orjinden uzaklaşan bulanık toplar(fuzzy balls) 

Sıstemin kod vektörlerini olabildiğince büyütmek istemesinin bir diğer sebebi de bulanık topların çakışmasını engellemek yani çözümleyicinin  farklı örtneklerin yeniden oluşturumu sırasında oluşabilecek karmaşayı engellemektir.
...
Topların boyurlarını da kontrol edebilmek mümkündür. Bu kontrol, varyansı 1'e yaklaştırmaya çalışarak toppu ne çok büyütüp ne de çok küçülten KL Iraksaması (KL Divergence)  ile yapılır.
