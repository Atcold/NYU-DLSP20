...
Bu yöntem, küçük değerler alan z'nin uzay hacmini sınırlayacak ve bu sınırlamayla düşük enerjiye sahip y'nin uzayını da kontrol ddebilmiş olacağız. Lambda'nın değeri bu dengeyi kontrol eder. Reel syılar kümesi $R$ için örnek vermemiz gerekirse , uygun boyutun neredeyse her yerde türevlenebilen  yakınsaması olan $L_1$ normu uygun bir adaydır. $z$'nin $L_2$ normunu kısıtlarken üzerine gürültü eklemek de bilgi kağasitesini sınırlar(VAE).
...
The $n$ boyutlu vector $z$, maximum sayıda sıfırdan farklı elemana will $m << n$ sahiptir. Böylece, her $Wz$, $W$'nun sütünlarının $m$ tanesinin oluşturdğu uzayın elemanı olucaktır.
...
Yukarıdaki kuralı yinelemeli sinir ağı gibi yorumlamak, yani saklı değişkken $z$'yi  sürekli tahmin edebilmemizi sağlayan parametreleri $W_e$ öğrenmek, mümkündür.Ağ, sabit sayıda $K$ adımında çalıştırılır ve  $W_e$'nin gradyanları zamanala geri yayılım yöntemiyle hesaplanır. Eğitilmiş ağ, FISTA algoritmasından daha kısa adımda iyi $z$'ler üretir. 
...
<b>Şekil 4</b>: LISTA as a recurrent net unfolded through time.
</center>
...
256 boyutlu bir saklı değişkene sahip seyrek kodlama sistemi, MNIST verisetinde kullanıldığında sistem eğitim kümesinin neredeyse tamamını doğrusal bir şekilde birleştirerek oluşturabilecek 256 adet kalem vuruşunu öğrenir. Seyrek düzenleştirici, yeni örnekleri oluşturuken olabildiğince az kalem vuruşu kullanmaya zorlar.
...
<b>Şekil 5</b>: Sparse coding on MNIST. Each image is a learned column of W.
 <!--MNIST'te Seyrek Kodlama. Her bir resim W'nin öğrenilmiş sütunudur. -->
</center>
...
Yukardaki şekil, renkli görüntülerde bir başka örnektir. Kod çözücü çekirdeğin (sağ tarafta) boyutu 9'a 9'dur. Bu çekirdek tüm resme evrişimsel bir biçimdfe uygulanır. SOldaki resim, kod çözücüdki seyrek kodları göstermektedir. $Z$ vektöründe bileşenlerin çok az beyaz veya siyahtır ki bu da vektörün oldukça seyrek olduğunu göstermektedir.
