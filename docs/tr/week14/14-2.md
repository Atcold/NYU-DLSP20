...
Cevap: Bölütleyici  önceden tanımlanmış sezgisel yintemlerle tasarlanmıştır. Model bölütleyicinin bu tanımına rağmen uçtan uca eğitilebilir hale getirilebilir. Bu el yapımı yaklaşımın yerini, karakter tanıma için kayan pencere yaklaşımı almıştır.
...
* Menteşe kaybı da is taking the energy of the most offending answer, and the correct answer, and computing their difference. Intuitively, with a margin m, the hinge will only have loss of 0 when the correct energy is lower than the most offending energy by _at least_ m.
...
### TANIM:
Bir çözücü *(decoder)* seslerin veya görüntülerin skorları/enerjileri ifade eden vektörler dizisini girdi olarak alır, ve oalbilecek en iyi çıktıyı seçer.
...
### 902ların ortasından kalma bir kontrol okuyucusu <!--
...
Kontrol okuyucusunün 90'ların ortasından kalma mimarisi oldukça karmaşıki fakat bizi ilgilendiren kısım, algılama çizgesini oluşturan karakter algılayıcıdan başlayan kısım.
<center>
<img src="{{site.baseurl}}/images/week14/14-2/check_reader.png"/><br>
<b>Figure 3</b>: Kontrol Okuyucusu 
</center>

Algılama çizgesi iki ayrı birleştirme işleminden geçen, biri doğru yorumlama( veya gerçek doğruluk) ile ve ikincisi ise tüm olası yorumlamaların çizgesini oluşturan gramer ile. 
...
İleri algoritma, tüm yollardaki negatif enerjilerin exponansiyelleri toplamının logaritmasını  aşağıdaki gibi hesaplar:  
$$F_{\beta} (x, y) = -\frac{1}{\beta} \; \log \; \sum_{z \, \in \, \text{paths}} \; \exp \, (- \beta \; E(x, y, z))$$

Diğer bir değişle, saklı değişken z üzerinden marjinalleştirme gerçekleştiriyor, bu işlem de algılama çizgesindeki yolları tanımlamamızı sağlıyor. Bu yöntem ile belli bir noktaya giden tüm yolların exponansiyelleri toplamının logaritmasını hesaplar. Bu tüm olası yolların bedellerini narin bir şekilde toplamak gibi.
...
Yapının altında yatan çizge zincir yapısına sahip ise, ileri algoritma aslında inanç yayılım *(belief propagation)* algoritmasının özel durumudur. Bu algoritma 

This entire algorithm can be viewed as a feed-forward neural network where the function at each node is a log sum of exponentials and an addition term. 
...
$\alpha_i$, sinir ağındaki $i$ düğümünün aktivasyonunu oluştururken $e_{ki}$ ise $k$ ve $i$ düğümleri arasındaki ağırlığı ifade eder. Bu formğlasyon cebirsel olarak . This formulation is algebraically equivalent to the weighted sum operations of a regular neural network in the log domain.
...
Geri yayılımın bu formülasyonu ile, yeni bir model üzerine konuşabiliriz. Neural ODE'ler, yinelemeli sinir ağları olup, durumu $z$'nin $t$ zamanındaki 
...
$$F\Bigg(\int_z q(z)\frac{\exp({-\beta E(x,y,z)})}{q(z)}\Bigg) \leq \int_z q(z)F\Bigg(\frac{\exp({-\beta E(x,y,z)})}{q(z)}\Bigg)$$
Şimdi, örnek olarak, konveks bir kayop fonksiyonu $F(x) = -\log(x)$  ile çalışalım:
$$-\log\Bigg(-\frac{1}{\beta}\int_z q(z)\frac{\exp({-\beta E(x,y,z)})}{q(z)}\Bigg) \leq \int_z q(z) * \frac{-1}{\beta}\log\Bigg(\frac{\exp({-\beta E(x,y,z)})}{q(z)}\Bigg)$$
$$\leq \int_z q(z)[E(x,y,z) + \frac{1}{\beta}\log(q(z))]$$
$$\leq \int_z q(z)E(x,y,z) + \frac{1}{\beta}\int_z q(z)\log(q(z))$$
...
Şimdi, karmaşık integrallerden kaçınabileceğimiz ve bunun yerine kendi seçtiğimiz  bir dağılımdan ($q(z)$) örnekleyerek bu değerleri yaklaşık olarak tahmin edebileceğimiz bir şekilde bir üst sınır formüle ettik!
