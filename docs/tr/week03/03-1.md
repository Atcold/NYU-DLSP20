...
-->
Figür 1 göselleştirmek istediğimiz sinir ağının yapısını betimliyor. Genellikle bir sinir ağının yapısını çizdiğimizde, girenleri aşağıda ya da solda; çıkanları yukarıda ya da sağda gösteririz. Figür 1'de, pembe nöronlar girenleri, mavi nöronlar çıkanları gösteriyor. Bu ağda, 4 adet saklı katman var (yeşiller), yani toplamda 6 tane katman var (4 saklı katman + 1 giriş katmanı + 1 çıkış katmanı). Bu örnekte, her gizli katmanda 2 nöron var, yani ağırlık matrisi ($W$)nın boyutu her katmanda 2 ye 2. Bunun nedeni giriş düzlemini görselleştirebileceğimiz bir düzleme dönüştürebilmek istememiz.
...
-->
Bu bize neden 2 nöronlu saklı katmanların daha zor eğitildiği hakkında fikir veriyor. 6 katmanlı ağımızın her saklı katmanında ek girdisi *(bias)* var. Bu nedenle eğer bu ek girdilerin herhangi biri sağ üstteki çeyrek düzlemin dışında bir yere işaret ediyorsa, ReLU operatörü bu noktaları sıfırlayacaktır. Sonrasında, ne kadar ileride olursa olsun, katmanların veriye yapacağı dönüşümler sonrasında bu noktalar sıfır kalmaya devam edecektir. Bir sinir ağını daha "şişman" yaparak (saklı katmanlara daha çok nöron ekleyerek) daha kolay eğitilmesini sağlayabiliriz, ya da daha çok saklı katman ekleyebiliriz, ya da bu iki yöntemin bir kombinasyonunu kullanabiliriz. Beklemede kalın, bu ders boyunca verilen probleme göre en iyi ağ yapısına karar vermeyi keşfedeceğiz.
...
-->
Genel olarak parametre transformasyonu, parametre vektörü $w$'nin bir fonksiyonun sonucu olduğu anlamına gerir. Bu transformasyonla, orijinal parametre uzayını başka bir uzaya eşleştirebiliriz. Figür 5'te, $w$, $H$nin $u$ parametresiyle olan halinin çıktısı. $G(x,w)$ bir ağ ve $C(y,\bar y)$ bir bedel fonksiyonu. Geri yayılım *(backpropagation)* formülü 
$$
...
-->
Bu formüller matris formunda uygulanıyorlar. Terimlerin boyutlarının birbirine uyması gerektiğine dikkat edin. $u$,$w$,$\frac{\partial H}{\partial u}^\top$,$\frac{\partial C}{\partial w}^\top$ , sırayla  $[N_u \times 1]$,$[N_w \times 1]$,$[N_u \times N_w]$,$[N_w \times 1]$ boyutlarına sahipler. Bu nedenle geri yayılım formülümüzün boyutları birbiriyle uyuşuyor.
...
### Kolay bir parametre trasnformasyonu: ağırlık paylaşma
<!--
...
-->
Paylaşılan parametrelerin eşit olmasını zorunlu kılıyoruz, böylece paylaşılan parametlere göre alınan gradyan (eğim), geri yayılımda toplanmış olacak. Örneğin bedel fonksiyonu $C(y, \bar y)$ 'nun $u_1$'a göre gradyanı, $C(y, \bar y)$'nın $w_1$ 'a göre olan gradyanı ve $C(y, \bar y)$'nın $w_2$'ye göre olan gradyanının toplamına eşit olacak.
...
-->
Kelimelerle ifade edersek, $i$'nci çıktı, **ters çevrilmiş** $w$ ile $x$ boyutundaki bir pencerenin arasındaki iç çarpım ile hesaplanabilir. Bütün çıktıyı hesaplamak için, pencere en baştan başlar, $x$ tamamlanana kadar bu pencere bir girdi kaydırılır ve bu şekilde tekrar edilir.
...
-->
Pratikte, derin öğrenme alanındaki PyTorch gibi yazılımlarda kabul edilen biraz farklıdır. Evrişim, PyTorch'ta $w$nin **ters çevrilmemiş** hali ile yazılmıştır:
$$y_i = \sum_j w_j x_{i+j}$$
<!--
...
-->
Matematikçiler bu formülasyona "çapraz korelasyon" derler. Bizim bağlamımımızda, bu fark sadece bir kural farkıdır. Pratik olarak çapraz korelasyon ve evrişim, hafızadaki ağırlıklar baştan sona ya da sondan başa okuyarak, birbiri arasında değişebilir.
Bunun farkında olmak önemlidir, mesela biri matematiksel bir makale üzerinden evrişim/korelasyon'un özelliklerinden faydalanmak isterse.
...
-->
Dolayısıyla doğada gözlemlediğimiz bu hiyerarşik ve birleşik yapı, sadece bizim görsel algımızın bir sonucu değil, fiziksel seviyede de doğru bir şey. En basit haliyle tanımlarsak, elimizde basit partiküller var, bunlar birleşerek atomları, atomlar birleşerek molekülleri oluşturuyorlar; bu şekilde ilerleyerek materyalleri, objelerin kısımlarını ve sonuçta da gerçek hayattaki bir objenin tamamını oluşturuyoruz.

Dünyanın bu birleşik yapısı, Einstein'ın insanların yaşadıkları dünyayı nasıl anladıkları ile ilgili retorik sorusunun cevabı olabilir:

> Evrenle ilgili en anlaşılmaz şey anlaşılabilir olmasıdır.

Yann'a göre, bu birleşik yapı sayesinde dünyanın insanlar tarafından anlaşılabilir olması hala bir komplo teorisi gibi duruyor. Yine de tartışılır, birleşme olmadan, insanların yaşadıkları dünyayı anlamaları için çok daha fazla sihir olması gerek. Büyük matematikçi Stuart Geman'ın sözleri ile:
...
-->
Yani neden Derin Öğrenme, dünyamızın anlaşılabilir ve birleşik bir yapısı olduğu fikrinin üzerine kurulmuş olmak zorunda olsun? Simon Thorpe'un araştırması bunu motive etmeye daha da yardımcı oldu. Etrafımızdaki objeleri tanımamızın aşırı hızlı olduğunu gösterdi. Deneylerinde her 100ms'de insanlara bir resim göstererek onların ne olduğunu söylemelerini istedi, ve insanlar da bunu başarıyla gerçekleştirdiler. Bu da insanların objelerin ne olduğunu 100ms gibi bir sürede anladığını gösterdi. Buna ek olarak, aşağıdaki diyagramda beyinin bir bölgesinden diğerine ne kadar sürede nöronlarla iletim olduğunu görebilirsiniz:
...
-->
Sinyaller retinadan LGN'ye (konstrast artırımı, kontrol gibi konularda yardımcı), sonra da V1 primer görme korteksi, V2, V4 ve sonrasında inferotemporal kortekse (PIT), yani beyinde kategorilerin tanımlandığı yere iletilir. Açık beyin ameliyatlarındaki gözlemlerin gösterdiğine göre, bir insana film izletildiğinde, PIT'deki nöronlar sadece belli görseller algılandığında ateşleniyorlar --Jennifer Aniston ya da anneannesini gördüğünde-- ve diğer hiçbir görselde bu olmuyor. Nöronların ateşlenmesi, pozisyon, boyut, ışıklandırma gibi şeylerle, anneannenizin hangi yöne baktığı, ya da ne giydiğiyle de değişmiyor.
...
Fukushima, basit ve karmaşık hücrelerin birden fazla katmanla bilgisayar modellerinde kullanılması fikrinin ilk sahibiydi, ve bunun için elle yazılmış rakam verilerini kullandı. Bu öznitelik algılayıcılarının bazıları özel olarak oluşturulmuş ve öğrenilmişti; öğrenme denetimsiz öbekleme algoritmalarını kullanmasına, her katman için ayrı ayrı eğitilmesine ve geri yayılımın henüz kullanımda olmamasına rağmen.

Yann LeCun birkaç sene sonra aynı yapıyı kurdu, ama bu sefer denetimli olarak geri yayılımla eğitti. Bu, evrişimli sinir ağlarının başlangıcı sayılmaktadır.
