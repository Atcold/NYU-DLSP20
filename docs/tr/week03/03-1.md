...
-->
Pratikte, derin öğrenme alanındaki PyTorch gibi yazılımlarda kabul edilen biraz farklıdır. Evrişim, PyTorch'ta $w$nin **ters çevrilmemiş** hali ile kodlanmıştır:
$$y_i = \sum_j w_j x_{i+j}$$
<!--
...
Fukushima, basit ve karmaşık hücrelerin birden fazla katmanla bilgisayar modellerinde kodlanması fikrinin ilk sahibiydi, ve bunun için elle yazılmış rakam verilerini kullandı. Bu öznitelik algılayıcılarının bazıları özel olarak oluşturulmuş ve öğrenilmişti; ama öğrenme denetimsiz öbekleme algoritmalarını kullanıyordu, her katman için ayrı ayrı eğitiliyordu ve geri yayılım henüz kullanımda değildi.
