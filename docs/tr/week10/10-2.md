...
Pretex görevler genelde özdenetimli olan ön eğitim adımlarını da kapsar ve sonrasında sınıflandırma veya sezim olabilen transfer görevlerimiz vardır. Ön eğitim görevi veya transfer görevlerinin "hizalanmış" olduğunu, yani, pretext görevlerini çözmenin transfer görevlerini iyi bir şekilde çözmede yardım edebileceğini *umarız*. Bu yüzden, birçok araştırma bir pretex görevi tasarlama ve onları iyi bir şekilde uygulama olarak ilerler.

Bununla birlikte, anlamsal olmayan bir görevin neden iyi öznitelikler üretmesi zor anlaşılan bir durum. Örneğin, Jigsaw bulmacaları gibi bir şeyi çözerkenneden anlamsal hakkındabir şeyler öğrenmeyi bekleyelimki? Veya neden resimlerden "hashtag'leri öngörmenin" transfer görevleri üzerinde bir sınıflandırıcı öğrenilmesine yardım edebileceği beklenebilir? Bu sebeble, bu soru devam ediyor. transfer görevleriyle iyi hizalanmış ön eğitim görevlerini nasıl tasarlayabiliriz? 

Bu problemi değerlenmede bir yol, her bir (bkz. Fig. 1) katmandaki gösterime bakmak. Eğer son katmandaki gösterimler transfer göreviyle iyi hizalanmış değilse, ön eğiitm görevi çözmede iyi bir görev olmayabilir. 
...
Fig. 2'de Jigsaw ön eğitimi kullanılarak VOC07 üzerindeki doğrusal sınıflandırıcılar için her bir katmandaki Mean Average Precision çizdirilmiştir. Son katmanın Jigsaw problemi için çok özelleştirilmiş olduğu çok açık.
...
### Ön eğitimli özniteliklerden ne istiyoruz?

* Resimlerin bir diğeri ile nasıl ilişki olduğunu göstermesini

  * ClusterFit: Görsel Gösterimin Genelleştirilmesinin Geliştirilmesi

* "Sıkıntılı faktörlere" karşı gürbüz olmasını -- değişmezlik

  Örn. nesnelerin tam yeri, ışıklandırma, tam renk

  * PIRL: Pretext Değişmez Gösterimlerin Özdenetimli Öğrenilmesi 

Yukarıdaki özelliklerin başarılmasını sağlayan iki yol **Kümeleme** ve **Karşıtsal Öğrenme**dir. Bu yöntemler bu zamana kadar tasarlanmış pretext görevlerden daha iyi bir performans sergilemeye başladı. Kümelemeye ait bir yöntem **ClusterFit** ve diğeri ise **PIRL**dir.


## ClusterFit: IGörsel Gösterimin Genelleştirilmesinin Geliştirilmesi

Öznitelik uzayını kümeleme hangi resimlerin diğer bir resimle alakalı olduğun görmek adına bir yoldur.


### Yöntem

ClusterFit iki adımı izler. Biri kümeleme adımı, ve diğeri öngörü adımıdır.

#### Kümeleme: Öznitelik kümeleme

Ön eğitimli bir ağı alır ve bir takım resimlerden bir grup özniteliği çıkartmak için kullanırız. Ağ bir tür ön eğitimli ağ olabilir. Sonrasında bu özniteliklere K-means kümeleme uygulanır, böylece her bir resim bir kümeye ait olur, ki bu küme resmin etiketi olmuş olur. 
...
#### Fit: Küme Atanmasının Öngörülmesi

Bu adım için, sıfırdan bir ağı resimlerin yalancı etiketlerini öngörmek için eğitiriz. Bu yalancı etiketler kümeleme sürecindeki ilk adımızda elde ettiğimiz şeylerdir.
...
Standart bir ön eğitim veya transfer görevi ilk olarak bir ağı ön-eğitir ve sonrasında Fig. 5'de görüldüğü gibi downstream görevinde değerlendirir. ClusterFit ön eğitimli $N_{pre}$ ağını elde etmek için $D_{cf}$ veri kümesi üzerinde bir ön eğitim gerçekleştirir. Ön eğitimli ağ $N_{pre}$ kümeleri üretmek için $D_{cf}$ veri kümesi üzerine uygulanır. Sonrasında bu veri üzerinde sıfırdan yeni bir $N_{cf}$ ağı öğreniriz. Son olarak bütün downsteram görevler için $N_{cf}$ kullanırız. 
...
### Neden ClusterFit Çalışıyor

Neden ClusterFit çalışıyor sorusunun cevabı kümeleme adımında sadece gerekli bilgi yakalanıyor, ve yapılar kısmen daha kapsamlı bir şeyler öğrenen ikinci bir ağ yapmak için atılıyor.

Bu noktayı anlamak için, gayet basit bir deney uygulanmıştır. ImageNet-1K, veri kümesine etiket gürültüsü ekledik, ve bu veri kümesinde bir ağ eğittik. Sonrasında ImageNet-9K üzerindeki bir downstream görevi üzerinde bu ağın öznitelik gösterimini değerlendirdik. Fig. 6'da görülebileceği gibi, ImageNet-1K verisine farklı büyüklüklerde etiket gürültüsü ekledik, ve ImageNet-9K üzerinde farklı yöntemlerin transfer performansını değerlendirdik. 
...
Pembe çizği ön eğitimli ağın performansını gösteriyor ki gürültülü etiket sayısı arttıkça azalıyor. Mavi çizgi 
The blue line represents model distillation where we take the initial network and use it to generate labels. Distillation generally performs better than pretrained network.  The green line, ClusterFit, is consistently better than either of these methods. This result validates our hypothesis.
